...
## 附：架构简化——声明式降级通道与 matmul 家族 pass 删除（第一阶段）

复核确认 matmul 全家族的变换均为单 op 局部机械映射（operand → 规划
地址、operand 类型 → tile token、accPhase → token、void bridge_call
发射、use-empty alloc 清理），无跨 op 状态，不需要家族 pass。本次把
分类轴从"家族"改为"变换复杂度"，白名单新增 `lowering` 字段分两档：

1. **声明式通道（`lowering: declarative`）**：新增通用 pass
   `PTOLowerDeclarativeBridgeOps`（func 级 nested，带 `whitelist-path`
   option），仅由白名单描述驱动：abi 行以 `operand` 位置索引绑定 IR
   operand（本工作区 MLIR 无泛化 ODS operand 名反射，`arg` 字段仅作
   诊断标签）+ `role` 作为 spec key 与 tmpl_map source；`tmpl_map` 的
   `source: attr` 行经自定义 AttrInterface `PTO_EnumTokenAttr`
   （`getEnumCaseSymbol()`）泛化反射枚举属性，拼白名单 `enum_type`
   前缀，`omit_value` 命中的 case（如 Unspecified）与属性缺失自然跳过
   （覆盖 mx.bias 无 accPhase 的形态）。spec key 派生规则（role 名、
   attr field 名、entry key = op 名去 `pto.t` 前缀 + `.` 换 `_`）精确
   复现原 `kBridgeSpec*Key` 常量，**WrapperGen 零改动**。
2. **家族通道（`lowering: family`，缺省）**：仅保留给有真家族语义的
   op（pipe 的 TPOP 地址重绑定、storage 生命周期、split 一致性）。
3. **删除 `PTOLowerMatmulFamilyOps.cpp`** 及其 Passes.td/Passes.h
   条目；pipeline 挂载点换成声明式 pass（保持先于 pipe 家族 pass 的
   顺序：先删掉已桥接的 tile 句柄）；未路由的 matmul op 仍保留 mad
   展开回退语义。
4. **共享 token 映射层**：抽取 `PTOCppTokens.{h,cpp}`（纯映射函数，
   限定符参数区分 `pto::` 全限定与空限定），EmitC 与桥接两侧改薄包装；
   组装层差异（NoneBox 省略、`_GM` 变体、GM/Zero 回退、ScaleLeft/
   ScaleRight 子角色）留在各后端，`vpto_bridge_emitc_token_parity`
   零 diff 验证。
5. **白名单 schema 扩展（向后兼容）**：`lowering`、abi 的
   `operand`/`arg`/`role`、tmpl_map 的 `enum_type`/`omit_value` 均为
   可选字段；删除 matmul 的硬编码 source 集（`isMatmulTmplMapSource`），
   declarative 条目改结构化校验（operand 不重复、tile source ∈ role 集、
   attr 行必须声明 enum_type）；pipe 的 source 集校验仅作用于 family
   条目。内置默认白名单六个 matmul 条目迁移到 declarative schema。
6. **attr 行属性名双拼写查找**：白名单 attr field 同时充当 spec key
   （`acc_phase`），而 ODS 属性名是 camelCase（`$accPhase`），声明式
   pass 查找时先按 field 原名、再按 camelCase 派生名取属性。
7. **测试迁移**：正向用例（family/variants lowering、wrapper 渲染）的
   CHECK 零改动，仅 RUN 行换成声明式 pass，输出逐字节一致；tmpl_map
   未覆盖诊断对 declarative 条目已结构性不可达（role 必被收集、source
   受 role 集约束），`whitelist_tmpl_coverage_diag` 重定向为解析期
   role 集校验诊断。新增 `vpto_bridge_declarative_binding_diag`
   （operand 越界/非 tile_buf/带 result/非枚举属性四场景）与
   `vpto_bridge_declarative_unrouted_passthrough`（未路由与 family
   条目放行）；fixture 全量迁移到 declarative schema；DEVICE=SIM
   `cube-matmul-bridge` compare passed。
