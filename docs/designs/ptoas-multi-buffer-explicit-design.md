...
| `PTOPlanMemory` N-way 多 slot 规划：`StorageEntry.relationOtherBuffers` 列表 + `ExpandMultiBufferStorageEntry` N-way 兄弟展开 + `UpdateBuffer2Offsets` 按 slot 顺序写回 | ✅ N ∈ [2, 16] | `lib/PTO/Transforms/Passes/PTOPlanMemoryModern.cpp`（原 `PTOPlanMemory.cpp`，legacy planner 删除后统一到 modern 实现） |
