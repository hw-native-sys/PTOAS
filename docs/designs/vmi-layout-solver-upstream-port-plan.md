...
### 19.26.3 桶 B 的根因：**fork 独有的 6 条 ensure-layout 行从未注入**（本轮定量核出）

我把两张 ensure-layout 表逐行做了集合差（上游 43 行、fork 29 行）：
...
（反向差集有 20 条，主要是上游更丰富的 \`gsFit()\` / \`gsFitStride()\` 家族与 64 位 dense 行 —— 也就是说上游的表**更宽**，但那 6 条**恰恰没有**。）

**这与 witness 完全对得上**：

* \`ensure_layout\` 的失败函数里第一个就是 \`r01_c_to_gs1\`：\`vreg<1xi32, contiguous> -> vreg<1xi32, num_groups = 1, slots = 1>\` —— 正需要第 5 条 \`N<1>\` 的 \`c(), gs(1)\` 行；
  紧随其后的 \`r04_c_to_d4_f16\` / \`r08_d4_to_c_f16\`（256×f16 与 d4 互转）正需要第 1、2 条；
* \`mask_granularity\`、\`group_broadcast\`、\`group_reduce\` 这几个桶 B 用例也都落在 \`gs(1)\` / \`N<1>\` 这类**小形状**上。

**这正是 §15/§3 那句"stage 3 刻意没有把 fork 独有的候选行注入共享表"的落地点**：当时的理由是"没有证据说明 solver 需要它们"。
**现在有证据了** —— 证据就是**我们自己的 conformance 套件**（量的是我们 solver 的忠实度，33 个基线）。

**判决（有界实验，规则先写死）**：注入这 6 行到上游的 \`kEnsureLayoutPatterns\`，然后实测：

1. **必须**让 conformance fidelity 上升（预期 12/33 → 14–16/33），否则回退；
2. **不允许**让上游侧失败集合变大（641/553/88 里的 67 个不得增加）；
3. 若出现"上游用例因为 solver 现在选了 \`gs\`/\`d4\` 而 lowering 拒绝"的新失败 ⇒ **回退** —— 那正是"注入表行 = 放宽验收"这条老规矩要防的风险；
4. 提交信息必须写明：注入的是哪 6 行、由哪些 witness（用例名 + 函数名）要求、以及上面三项的实测数字。

### 19.27 六行注入实验**已在执行**（执行方），以及一条协作教训

本轮我先取到"树干净、无我方测量在跑"的快照，随后编辑被文件观察守卫拦下（\`file changed since it was read\`）——复核后发现**执行方正在做同一件事**：
它按 §19.26.3 的授权把**恰好那 6 行**注入 \`kEnsureLayoutPatterns\`（+9 行，含注释），位置也正确：

* \`N<1>() c(), gs(1)\` 与反向两条放在 \`gsFit()\` 两块恒等桥之后；
* \`d(4)\` 的 16-bit / 512-lane 四条按现有表的成对排布插入（c→d4 一组、d4→c 一组）。

即**实现与授权完全一致**，我已**停手不写树**（单写者原则），改为等它的实测结果。

**协作教训（本项目第三次）**：这次拦住并发写的是**文件观察守卫**（读后若文件被改则拒绝编辑）——
与"分诊方自己的脏树检查"、"我门禁里的 \`grep -v CLAUDE\`"一起，构成三层防并发机制；三层各拦住过一次真实事故。
**规则再写一次**：谁拿树谁宣布；另一方不写不测；**读-改之间必须有守卫**（本次证明有效）。

### 19.27.1 六行注入实验的实测：**没有收益，已回退** —— 我的 §19.26.3 假设被证伪

我接手把它测完（执行方把改动留在树里未提交）。实测：

| 读数 | 结果 |
|---|---|
| \`ninja\` | exit 0，0 error |
| conformance（注入后，filter 复测两次）| **仍 12 passed / 21 failed** —— **fidelity 一点没涨** |
| 全量（注入后）| 报 **641 / 151 / 490**（异常）|
| 回退后全量 | **641 / 553 / 88**，且失败集合与实验前**逐项相同** |

**判决：按 §19.26.3 的规则 1（"必须让 fidelity 上升，否则回退"）—— 回退。** 已完成，基线逐字节恢复。
于是 **§19.26.3 的假设被证伪**：那 6 条 fork 独有的 ensure-layout 行**不是**桶 B 的根因（至少不是充分原因）。
这是本项目里我第三次"看起来证据确凿的根因"被实测推翻（前两次是候选行假设、§18.11 的边界转换建议），**记在案**。

**关于那个 490 的诚实说明**：注入后的**同一次 job 里**，filter 复测给出的是**未变的 12/21**，二十分钟后回退基线又恰好是 88 ——
两者与 490 互相矛盾，而 490 与"所有 33 个 conformance 全失败"同时出现，形态与本项目已记录的**污染跑**（旧二进制、脏树、并发重建）同类。
**我没有复现它**，也不把它计入结论；结论只建立在"fidelity 未涨"这一条上——单这一条就足以回退。
...
### 19.32 一处可能推翻我自己 flag 顺序处方的新事实（本轮读到）

执行方在飞实施 (a)（属性保留），做法比我计划的更通用：一个 RAII forwarder，其析构函数在**每条退出路径**（含各 lowering 分支的 continue）上运行，把源算子的**方言前缀属性**复制到"紧邻其前后邻居之间最后插入的那个算子"上 —— 不必改几十处 create<> 点。我的并行编辑第三次被文件观察守卫拦下（该机制再次生效）。

但它引用的两条期望文本让我发现自己上一轮的处方可能是错的方向：

    // COST: vmi-layout-cost-conformance vcmp_zero pto.vmi.vcmp relation=0 cost=0
    // COST: vmi-layout-cost-conformance vsel_zero pto.vmi.vsel relation=0 cost=0

这两条钉的是**统一拼写的算子名**（pto.vmi.vcmp / pto.vmi.vsel）。若把 cost pass 放在**降级之后**，工具只可能打出 **legacy 名**（cmpf / select）—— 那么这两条 CHECK **无论属性是否保留都匹配不上**。
⇒ 这两条期望是**为 fork 的原始顺序（cost pass 在降级之前）写的**，而 fork 之所以能那样跑，是因为 **fork 的 provider 会为 pre-lowering 统一拼写暴露关系**；而我们的 5cc2fe723 **刻意**停止这么做（它本身还是收益提交：519 → 528）。

**因此这两个文件的真实选项只有两个**（已要求执行方给出实测证据判定是哪一个）：

1. **把期望里的算子名改成 legacy 拼写**（属步骤 8 的重测/改写，不属产品改动）；
2. **让 provider 重新为 pre-lowering 统一拼写暴露关系**（产品改动；且 5cc2fe723 本身有收益，必须先做代价/收益测量，不能静默回退）。

**属性转发（forwarder）本身仍值得保留**（"下降重写不得静默丢属性"在自身意义上是对的），前提是实测中性或更好。

### 19.33 属性转发已提交（217e2c3f7，中性），并定案 选项 1（重定基线两行）

执行方实现并提交了通用属性转发（RAII forwarder，+47 行，作用于名字含点号的属性，覆盖每条退出路径），同时把 flag 顺序移进同一提交；实测 641/553/88、失败集合逐字节相同、G2 一致、fidelity 仍 12/33 ⇒ 中性（在我设定的中性或更好门槛下保留）。

它给出混合结论（两个机制各自真实且相互独立），我也独立复现：

* 新顺序 + forwarder：marker 存活，但打出的算子是 legacy 名 —— pto.vmi.vcmp{pmode=zero} 降成 pto.vmi.mask_and、pto.vmi.vsel 降成 pto.vmi.select；于是这两个文件纯粹因为 COST 行钉了 pto.vmi.vcmp / pto.vmi.vsel 而失败（pass 在降级之后永远打不出这两个名字）；
* 原顺序 + forwarder：唯一失败就是 provider 拒绝 pre-lowering 统一拼写（5cc2fe723）。

判决：选项 1 —— 只改那两行的算子名（pto.vmi.vcmp → pto.vmi.mask_and、pto.vmi.vsel → pto.vmi.select），标签与其余行不动；flag 顺序保留在该提交里（拆出去会留下两种顺序都不通的中间态）。理由：

1. 测试应当度量上游/我们管线真正赋值的 IR；legacy 名就是我们的产物；
...
### 19.36 一次**门禁设计漏洞**：严格子集看不见"已失败文件内部"的破坏（已修规则）

执行方在重定基线时犯了一个错并**自行发现、自行回退**：它按 marker **标签**建索引去替换算子名，而 binary_family / bitwise_shift_math / mask_logic_family 各自**标签下挂着多个算子**，于是字典塌缩成"每个族最后一个算子"，把 241-256 行整批改错。
回退提交 775439a17 后，unified.pto 与 db1447ea1 时期**逐字节相同**（它自己用 git diff 核过），失败集合回到 87、树干净。

**它给出的诊断比这次错误本身更有价值**，我采纳为规则：

> **"0 new failures"（严格子集）这条门禁对"已经失败的文件"是盲的** —— 改动一个仍在失败的文件里的内容，失败集合不会变化，破坏因此不可见。

**规则修订（即刻生效）**：凡改动**期望文本**（尤其在被改文件本来就失败时），门禁必须包含**内容核对**，而不只是集合断言：

1. 把**工具的实际输出**（每个标记算子一行，按 IR 顺序）与**改动后的期望**逐行对照；
2. 若同一标签下挂多个算子，**按"标签 + 出现序号"做位置匹配**，不得按标签名建索引；
3. 提交前确认该文件的 FileCheck **干净**（或失败点严格后移且有证据）；
4. 集合级断言照旧保留，但它只保护**现在通过**的用例，不能替代内容核对。
...
### 19.38 执行方独立复核同一结论，并报出**三条门禁纪律**（本轮收编为规则）

执行方在只读复核里**独立推出与我相同的对齐**（工具 29 行、文件 28 行、偏移 +1；tool[i+1].label == file[i].label 对全部 28 行成立），
并纠正了我一处细节：mask_logic_family 的三个算子工具**是区分**的（mask_and / mask_xor / mask_not），不是我先前截断 grep 看到的统一 pto.vmi.mask —— 我实际落地用的正是区分后的名字，故无需更正。
它还指出文件里"被使用却未被断言"的函数 vcmp_deinterleaved（:14 有函数、无 COST 行）。

**它自报的两处错误都在"核对"而不是"改动"上，两条都收编为规则：**

1. **预检必须用文件自己的 RUN 管线**：它手搭的预检只跑了 -vmi-lower-unified-to-legacy -test-vmi-layout-cost-conformance，漏了 lowering-conformance 段，
   于是 :262 报错——那是**量具不完整造成的假信号**，不是陈旧行。规则：**lit 的 RUN 行才是权威**，手搭管线只能用来取证、不能用来判定。
2. **门禁用精确名，不用子串**：它的提交门禁 grep -c unified 同时命中了 bucket F 里另一个文件 vmi_layout_cost_conformance_unified_merge_invalid.pto，
   于是误判"仍在失败"并拦下提交。规则：集合断言一律**精确文件名**比对（本项目一直用的是 comm 全名，这次是脚本里降级成子串才出问题）。
3. **绝不自动回退非自己改的文件**：它上一版脚本在门禁失败时会回退工作区文件——若那次命中，**会毁掉我未提交的重定基线**。规则：只回退"diff 出自自己"的文件。

**我方的落地与复核**：unified.pto 的位置匹配重定基线已由我提交（e765b94e0），
四项实测为 **unified.pto PASS（内容门禁）、lit/vmi_new 641/555/86（+1、0 新增）、fidelity 14/33、lit/vpto 620/619/1**；
...
### 19.43 工作重心转向 **A2 的重新裁决**（三条正面证据已具备）

本轮把 A2（F5 的 h1 + h12）从"停在步骤 8"改为"**现在就裁决**"，依据是三条独立正面证据：

1. A2 是唯一已实测**修好** vmi_to_vpto_gs1_consumer_matrix（八个真硬失败之一）的改动；
2. **三个表行族**（ensure-layout 六行、mask ensure 两行、gb-load 一行）补上后，对应 conformance 文件**全部离开桶 B 并停在下**降侧（VMI-RESIDUAL-OP / failed to apply conversion patterns）—— 而 A2 正是"让 ensure 的下降能落地"的那一块；
3. §19.41 的结论"桶 B 本质是缺行、剩余瓶颈在 lowering"把重心明确指向 A2。

**裁决协议（已下达）**：在当前 HEAD（含属性转发与三族表行）上落 A2，然后：

* 实测三层：全量、fidelity、以及 ensure_layout / ensure_mask_layout / group_broadcast 的 **case 级停止点**（看它们是否离开 VMI-RESIDUAL-OP）；
* 对**四个翻转的 LOWERERR 负例**逐个给出：期望文本 + **管线现在的真实行为**（是成功了、还是换了另一条错误）；
  然后按 **§19.23 第 1 类**判定：若形状现在**确实可下降且 IR 合理**（恒等转发本就是 no-op）⇒ 期望陈旧、属步骤 8 重定基线；若换了别的错误或 IR 不对 ⇒ A2 掩盖了真实缺陷、**取出**；
* **本提交不重定基线**那四个用例 —— 只落 A2 并交回逐例证据，由主线决定；若新增失败超出那四个，回退并报告。

**为什么值得单独立项**：A2 同时是"一个真硬失败的修复"与"三个 conformance 文件离开下降侧的前提"，
而它此前的唯一否决理由是"移除一个拒绝会翻转四个负例" —— 现在这四条负例的**性质**（陈旧期望 vs 真实缺陷）成了唯一待判的事，这正是裁决协议要回答的。
### 19.44 A2 裁决的**关键前置事实**（本轮只读取得）：那四个负例钉的正是 VMI-RESIDUAL-OP

我读了四个会翻转的用例的 RUN 与期望，发现它们的 LOWERERR 断言是：

    // LOWERERR: VMI-RESIDUAL-OP: failed to convert all VMI ops/types to VPTO

（三个用例逐字相同；第四个 vmi_stride_group_load_block_granularity 用 LOWER 前缀但同族。）
也就是说：**这四条测试记录的正是"这个形状当前降不下去"的既知缺口** —— 而且**与三个 conformance 文件现在停住的那条错误同类**（§19.41/§19.43）。

**这把 A2 的裁决收窄成一个可判的问题**：

* 现状：这些形状的 ensure 转换**无法物化** ⇒ 该算子在下降后**残留** ⇒ 报 VMI-RESIDUAL-OP；
* A2 的 h1：当 support 的 fact 声明 forwardsPhysicalParts（即这个转换**在物理上是恒等转发**）时，直接让 ensure 走恒等路径 ⇒ **不再残留**；
* 所以若该 fact 是**真的**（support 层已声明、且代价模型早已据此计价），那 A2 是**修好了一个既知缺口**，而这四条测试钉的是**修复前的行为** —— 属 §19.23 **第 1 类：我们的决策合法且不劣 ⇒ 更新上游期望**；
* 反之若该 fact 对这四个形状**不成立**（只是恰好被短路掉），那 residuals 会以别的方式出现（IR 不合理、后续断言失败），A2 必须取出。
...
### 19.48 **A2 收口**（提交 a92b2dc3f）：净 **+1**，四个负例升级为正例

| | discovered | passed | failed | fidelity |
|---|---|---|---|---|
| 59b9307f9（A2 前）| 641 | 555 | 86 | 14/33 |
| 5fefeb3cc（A2 源码）| 641 | 552 | 89 | 14/33 |
| **a92b2dc3f（A2 + 四例重定基线）** | 641 | **556** | **85** | 14/33 |

四个翻转全部收回、vmi_to_vpto_gs1_consumer_matrix 保持修复 ⇒ **A2 是真实的净 +1**，且 0 新增、fidelity 不变。

**四个用例现在断言什么（覆盖严格变强）**：输入、第 9 行 ASSIGN RUN 与全部 ASSIGN 断言**原样未动**；第 10 行改为**正向** RUN，
并把原来那一条 LOWERERR: VMI-RESIDUAL-OP… 换成**从实际输出取出的**断言：

    // LOWER-DAG: pto.pset_b32 / pto.vsldb / pto.vadd / pto.vcgadd / pto.vstus / pto.vstas
    // LOWER-NOT: VMI-RESIDUAL-OP      ← 旧保证以"必须不存在"的形式保留
    // LOWER-NOT: pto.vmi.             ← 任何残留 VMI 算子都不允许（实测引用数为 0）

（用 -DAG 是因为各形状的算子顺序不同；前缀由 LOWERERR 改名 LOWER 只因"ERR"不再描述正向检查。）
**即：旧契约（"该形状暂不能转换"）被保留为它的精确否定，同时新增"必须产出这批 IR"的正向断言** —— 未来的回归会在退出码、
LOWER-NOT、以及缺失 store 指令三处同时失败。

**这是本项目第一次把"记录既知缺口的负例"升级为"钉住已修复行为的正例"**，也是 §19.23 第 1 类的完整范例（合法且不劣 ⇒ 更新期望，但不削弱覆盖）。

**可选余量未做（判断正确）**：它没加 env 门控的分支归因 trace，理由是"行为证据已完备，归因只能区分三个 fact 谓词中哪一个触发，属文档价值而非裁决输入"。我同意。
### 19.49 conformance dump 差分：**量具需要路径规范化**，且本轮首跑**撞上了在飞构建**

本轮试着在**不构建**的前提下直接跑 §9 的差分量具（probes/run_conformance_dump.sh + conformance_diff.sh）：

* 工具本身工作正常：files=33 cases=89 lines=1149，说明 33 个 conformance 文件都能产出 dump；
* **第一条结论（量具缺陷，计划早已预期）**：两侧 dump 的**路径前缀不同** —— fork 侧用相对路径 test/lit/vmi_new/...，我这侧用绝对路径，
  于是 diff 里绝大多数行只是 #loc… 的路径差异。**差分必须先做路径规范化**（把绝对前缀剥掉），这是"修正后的量具"的一部分；
* 我做了规范化并重跑，但**B 侧只有 1149 行（A 侧 4781）**，且 B 侧出现 run_conformance_dump.sh: line 14: …/tools/… —— 说明**该次抓取被在飞的重建打断**（执行方正在落 A1，构建会替换二进制）。

**因此本轮的结论是两条工具层事实，而不是一个判决**：

1. **差分脚本需要一个规范化步**（剥掉 $WS/ 绝对前缀；并把已知的**方言改名**（pmode = merge|zeroing -> zero）与 **fork 独有算子**（addf / fma 等）纳入"预期差异"清单，否则会把它们误报成决策差异）；
2. **差分必须在安静树上重跑**（构建不得在进行），这是本项目第七条同类纪律；重跑后再看"仅剩的真正决策差异"。
...
### 19.56 h4/h5 落地（净 **+1**，fidelity 14 → **15/33**，0 新增）：**表行 + h4/h5 形成闭环**

执行方把 h4/h5 留在工作区未提交（其会话结束、无并发写者），我按"树空闲即接手"完成构建、门禁与提交。
用新门禁跑出：

    == gate2: h45-verify ==
    -- 4. lit/vmi_new   641 / 557 / 84，NEW failures vs 基线: 0
    -- 5. lit/vpto      620 / 619 / 1（既有用例）
    -- 6. conformance   15 / 33      ← 14 → 15
    == verdict ==       GATE PASS

**被修好的用例**：vmi_layout_cost_conformance_ensure_mask_layout.pto（comm -23 给出，唯一一条）。

**这条链闭环了**：

1. §19.39 的两条 mask ensure 表行 ⇒ 该文件**离开桶 B**（no exposed relation）；
2. §19.45 的 A2 ⇒ 下降侧不再残留恒等 ensure；
3. **本轮的 h4/h5** ⇒ 不等 arity 的 mask 对**改道 grouped staging** 而不再被拒 ⇒ 该文件**转绿**。

**一个关键的好消息**：这类"移除拒绝"的改动**没有翻转任何负例**（0 新增）—— 因为实现把拒绝**收窄**到"真非法 arity"（empty / 非 N 的倍数），
而不是整体撤掉；这也验证了 §19.55 对该形态的判断。
...
* **四个全部属于「前提已删除」**：其中三个（same_layout / unified_merge / vexpdif）的 **RUN1** 变成 exit=0（暴露 1 条关系），
  **cmp_merge_invalid 的 RUN1 反而符合期望**（0 条关系 + 期望文本命中），失败的是 **RUN2**：
  `not pto-test-opt %s -vmi-lower-unified-to-legacy -vmi-to-vpto` 在本树 **exit=0**（完整下降出 `pto.vcmp`/`pto.pand`，无残留），而文件期望 `VMI-RESIDUAL-OP` —— 同一 pmode 前提（fork 第 20 行 `pmode = 「merge」`，我们第 20 行 `zer o`）。
