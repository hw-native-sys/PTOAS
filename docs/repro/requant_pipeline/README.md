...
The repeated-looking reduction statements in `requant_stage_vmi.py` are also intentional. On the pinned PTODSL build, replacing those statements with `pto.static_range` changes the emitted module from about 1,022 lines to more than 10,000 lines by unrolling the active vector body. That is not code-generation equivalent and can change register pressure and performance, so the checked-in fixture retains the compact, verified lowering.
