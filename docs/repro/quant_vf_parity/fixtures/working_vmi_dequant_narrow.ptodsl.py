# Control schedule dump twin for Ask 2 (narrower K face matched AscendC on device).
# Same algorithm family as broken_vmi_dequant_dblbuf.ptodsl.py; see RECORDED_DEVICE_NOTES.md.
...
@pto.jit(name="dequant_dblbuf_narrow", kernel_kind="vector", target="a5", mode="explicit")
def dequant_dblbuf_narrow(out: pto.ptr(pto.f32, "gm"), x: pto.ptr(pto.f8e4m3, "gm"), x_sf: pto.ptr(pto.ui8, "gm")):
