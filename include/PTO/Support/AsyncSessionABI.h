...
// Field positions in the session config struct.
//
// This layout is internal. It does not appear in the IR a user writes: a
// session is spelled `!pto.dma_session<sdma>` there, and the wrapper expansion
// materializes the struct below from this enumeration, addressing it with
// `pto.struct_get` / `pto.struct_set`. Adding or moving a field therefore costs
// a rebuild of the expansion and of the host template builders, and nothing
// else.
//
// The engine is not a field. It is a parameter of the session type, which is
// what lets an SDMA kick reject another engine's session by type rather than by
// reading a field back out; `SessionEngine` below stays only so a host that
// writes a template names the same engine the type does. There is likewise no
// validity flag: a session value exists only where `pto.session_init` produced
// it, so there is no invalid state left to describe.
//
...
enum class SessionField : int64_t {
  ...
  BlockBytes = 6,
  DestRankId = 7,
  // Queue pair for the connection-oriented engines. SDMA does not read it; it
  // is kept here so a URMA or RDMA session can be added without moving the
  // fields around it.
  QpIdx = 8,
...
// Width of each field, in the same order as the enumeration. The expansion
// builds the session struct type from this, so the widths live here next to the
// positions instead of being restated in the pass.
constexpr unsigned kSessionFieldWidths[kSessionNumFields] = {
    64, // ContextGm
    64, // TmpBufAddr
    32, // TmpBufSize
    32, // SyncId
...
constexpr unsigned sessionFieldWidth(SessionField field) {
  return kSessionFieldWidths[sessionFieldIndex(field)];
}

// Mirrors mlir::pto::DmaEngine, the parameter of !pto.dma_session. A host that
// writes a template does not put this in the template -- the engine is fixed by
// the session type the kernel builds -- but it still needs to name the engine
// when it asks the runtime for channels.
...
// 40..63 reserved.
//
// Nothing is needed here to wait on a post, which is why pto.sdma_gm_gm hands
// back this record rather than a bare address. The engine owns the head and the
// poster owns the tail, and both addresses are above, so
//
//   *head == *tail
//
// says the channel has drained -- everything posted on it has retired. That is
// coarser than per-post completion, since a later post on the same channel
// pushes the tail out again, but it is never early, and posts on a channel
// retire in order anyway.
//
// A stable predicate is the reason to prefer it over comparing the head against
// a remembered tail: once drained it stays drained until the next post, while an
// equality against a saved value is a moment a slow poller can step over.
