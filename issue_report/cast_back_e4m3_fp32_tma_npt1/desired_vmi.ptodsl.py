  ...
  for w in range(0, ((num_tokens + 2303) // 2304), 1):
    ...
    if ((w * 72) + pto.get_block_idx()) < ((num_tokens + 31) // 32):
      ...
      if 0 < ((sf_extent - (pto.get_block_idx() * 64)) - (w * 4608)):
        pto.mte_gm_ub(pto.addptr(x_sf, (scalar.cast(w, pto.si64) * 4608) + (scalar.cast(pto.get_block_idx(), pto.si64) * 64)), pto.addptr(buf_dyn_shmem, 4096), 0, scalar.min(64, ((sf_extent - (pto.get_block_idx() * 64)) - (w * 4608))), nburst=(2, sf_stride, 64))
      ...
      with pto.vecscope():
        ...
        for local_block in range(0, 32, 1):
          for scale_i in range(0, 4, 1):
            loaded = pto.vmi.vload(pto.addptr(buf_dyn_shmem, 4096), ((((scale_i // 2) * 64) + (local_block * 2)) + (scale_i & 1)), size=1)
            u16 = pto.vmi.vcvt(loaded, to_dtype=pto.ui16)
            shifted = pto.vmi.vshls(u16, 7, one)
            is_inf = pto.vmi.vcmps(shifted, 32640, one, "eq")
            pto.vmi.vstore(pto.vmi.vinterpret_cast(pto.vmi.vsel(is_inf, nan1, shifted), to_dtype=pto.bf16), pto.addptr(pto.castptr(buf_dyn_shmem, pto.ptr(pto.bf16, "ub")), 2112), ((local_block * 64) + scale_i), one)
      pto.set_flag("V", "MTE2", event_id=1)
      ...
      with pto.vecscope():
        ...
        for row in range(0, 32, 1):
          for half in range(0, 2, 1):
            scale = pto.vmi.vbrc(pto.vmi.vload(pto.addptr(pto.castptr(buf_dyn_shmem, pto.ptr(pto.bf16, "ub")), 2112), ((row * 64) + (half * 2)), group=2, size=2, stride=1), group=2, size=64)
            source = pto.vmi.vload(pto.castptr(buf_dyn_shmem, pto.ptr(pto.f8e4m3, "ub")), ((row * 128) + (half * 64)), size=64)
            value = pto.vmi.vcvt(source, to_dtype=pto.f32)
            scale_f32 = pto.vmi.vcvt(scale, to_dtype=pto.f32)
            pto.vmi.vstore(pto.vmi.vmul(value, scale_f32, mask), pto.addptr(pto.castptr(buf_dyn_shmem, pto.ptr(pto.f32, "ub")), 2080), ((row * 128) + (half * 64)), mask)
