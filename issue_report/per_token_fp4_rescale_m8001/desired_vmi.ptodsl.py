# Desired VMI for ASC per_token fused rescale at M=8001 (no host pad).
#
# Production aligned fused rescale (tilelang_dump.ptodsl.py) is already legal
# 1×T. ASC uses Persistent ceildiv(num_tokens, 16) and clamped DMA.
# The missing tail is an in-kernel zero of unused e4m3/e2m1 UB rows:
#
#   T.clear(x_ub)                         # TileLang: Bad bit-width for float8_e4m3fn
#   pto.vmi.vbrc(pto.f8e4m3(0), size=256) # PTODSL: unsupported eager constructor f8E4M3FN
#
# PTOAS / TileLang must accept one of those on a legal 1×T mask
# (create_mask width in {1,64,128,256}) so VMI can match ASC without host pad.
#
# The remainder of this file is the working aligned dump once copied from
