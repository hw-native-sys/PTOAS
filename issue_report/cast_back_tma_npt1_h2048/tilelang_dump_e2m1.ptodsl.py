...
def main_kernel(out: pto.ptr(pto.bf16, "gm"), x: pto.ptr(pto.f4e2m1x2, "gm"), x_sf: pto.ptr(pto.ui8, "gm"), num_tokens: pto.si32, sf_extent: pto.si32, sf_stride: pto.si32):
  ...
  for w in range(0, ((num_tokens + 4607) // 4608), 1):
    ...
    if ((w * 72) + pto.get_block_idx()) < ((num_tokens + 63) // 64):
      for pid_k in range(0, 4, 1):
        ...
        if 0 < ((sf_extent - (pto.get_block_idx() * 128)) - (w * 9216)):
          pto.mte_gm_ub(pto.addptr(x_sf, ((scalar.cast(w, pto.si64) * 9216) + (scalar.cast(pto.get_block_idx(), pto.si64) * 128)) + ((scalar.cast(pid_k, pto.si64) * scalar.cast(sf_stride, pto.si64)) * 8)), pto.addptr(buf_dyn_shmem, 32768), 0, scalar.min(128, ((sf_extent - (pto.get_block_idx() * 128)) - (w * 9216))), nburst=(8, sf_stride, 128))
        pto.set_flag("MTE2", "V", event_id=2)
        pto.wait_flag("V", "MTE2", event_id=x_ub_version_counter_1 & 1)
        pto.mte_gm_ub(pto.addptr(x, ((scalar.cast(w, pto.si64) * 4718592) + (scalar.cast(pto.get_block_idx(), pto.si64) * 65536)) + (scalar.cast(pid_k, pto.si64) * 256)), pto.addptr(pto.castptr(buf_dyn_shmem, pto.ptr(pto.f4e2m1x2, "ub")), (x_ub_version_counter_1 & 1) * 16384), 0, 256, nburst=(scalar.min(64, scalar.max(((num_tokens - (pto.get_block_idx() * 64)) - (w * 4608)), 0)), 1024, 256))
        ...
        with pto.vecscope():
          ...
          for local_block in range(0, 64, 1):
            for scale_i in range(0, 16, 1):
              loaded = pto.vmi.vload(pto.addptr(buf_dyn_shmem, 32768), ((((scale_i // 2) * 128) + (local_block * 2)) + (scale_i & 1)), size=1)
              u16 = pto.vmi.vcvt(loaded, to_dtype=pto.ui16)
              shifted = pto.vmi.vshls(u16, 7, one)
              is_inf = pto.vmi.vcmps(shifted, 32640, one, "eq")
              pto.vmi.vstore(pto.vmi.vinterpret_cast(pto.vmi.vsel(is_inf, nan1, shifted), to_dtype=pto.bf16), pto.addptr(pto.castptr(buf_dyn_shmem, pto.ptr(pto.bf16, "ub")), 16896), ((local_block * 64) + scale_i), one)
        pto.set_flag("V", "MTE2", event_id=2)
        ...
        with pto.vecscope():
          ...
          for row in range(0, 64, 1):
            for half in range(0, 4, 1):
              scale = pto.vmi.vbrc(pto.vmi.vload(pto.addptr(pto.castptr(buf_dyn_shmem, pto.ptr(pto.bf16, "ub")), 16896), ((row * 64) + (half * 4)), group=4, size=4, stride=1), group=4, size=128)
              source = pto.vmi.vload(pto.addptr(pto.castptr(buf_dyn_shmem, pto.ptr(pto.f4e2m1x2, "ub")), (x_ub_version_counter_1 & 1) * 16384), ((row * 256) + (half * 64)), size=(128 // 2))
              value_bf16 = pto.vmi.vinterpret_cast(pto.vmi.vcvt(source, to_dtype=pto.vmi.bf16x2), to_dtype=pto.bf16)
              result = pto.vmi.vmul(value_bf16, scale, mask)
              pto.vmi.vstore(result, pto.addptr(pto.castptr(buf_dyn_shmem, pto.ptr(pto.bf16, "ub")), ((x_ub_version_counter_1 & 1) * 32768) + 20992), ((row * 512) + (half * 128)), mask)
