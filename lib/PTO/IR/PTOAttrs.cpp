...
struct BridgeTileSpecParts {
  TypeAttr element;
  DenseI64ArrayAttr shape;
  DenseI64ArrayAttr validShape;
  AddressSpaceAttr memory;
  IntegerAttr bLayout;
  IntegerAttr sLayout;
  IntegerAttr fractal;
};

static FailureOr<BridgeTileSpecParts>
getBridgeTileSpecParts(DictionaryAttr tile) {
  BridgeTileSpecParts parts{};
  parts.element = tile.getAs<TypeAttr>("element_type");
  parts.shape = tile.getAs<DenseI64ArrayAttr>("shape");
  parts.validShape = tile.getAs<DenseI64ArrayAttr>("valid_shape");
  parts.memory = tile.getAs<AddressSpaceAttr>("memory_space");
  parts.bLayout = tile.getAs<IntegerAttr>("b_layout");
  parts.sLayout = tile.getAs<IntegerAttr>("s_layout");
  parts.fractal = tile.getAs<IntegerAttr>("s_fractal");
  bool missing = !parts.element || !parts.shape || !parts.validShape ||
                 !parts.memory || !parts.bLayout || !parts.sLayout ||
                 !parts.fractal;
  if (missing) {
    return failure();
  }
...
verifyBridgeTileSpecLayout(const BridgeTileSpecParts &parts,
                           function_ref<InFlightDiagnostic()> emitError,
                           StringRef fieldName) {
  bool rankMismatch = parts.shape.size() != 2 || parts.validShape.size() != 2;
  if (rankMismatch) {
    return emitError() << fieldName << " requires rank-2 shape and valid_shape",
           failure();
  }
  for (auto [extent, valid] :
       llvm::zip(parts.shape.asArrayRef(), parts.validShape.asArrayRef())) {
    if (extent <= 0 || valid <= 0 || valid > extent) {
      return emitError() << fieldName << " has invalid shape/valid_shape",
             failure();
    }
  }
  bool invalidLayout = parts.bLayout.getInt() < 0 ||
                       parts.bLayout.getInt() > 1 ||
                       parts.sLayout.getInt() < 0 ||
                       parts.sLayout.getInt() > 2 ||
                       parts.fractal.getInt() <= 0;
  if (invalidLayout) {
...
verifyBridgeTileSpec(DictionaryAttr tile,
  ...
  FailureOr<BridgeTileSpecParts> parts = getBridgeTileSpecParts(tile);
  if (failed(parts)) {
    return emitError() << fieldName
                       << " has missing or incorrectly typed fields",
           failure();
  }
  if (!isBridgeElementType(parts->element.getValue())) {
    return emitError() << fieldName << " has unsupported element type "
                       << parts->element.getValue(),
           failure();
  }
  return verifyBridgeTileSpecLayout(*parts, emitError, fieldName);
}
