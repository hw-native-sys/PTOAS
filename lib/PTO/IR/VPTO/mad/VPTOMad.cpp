...
// Parse the fixed shape types: lhs, rhs, dst, [bias], m, n, k.
static ParseResult parseMadFixedTypes(OpAsmParser &parser, bool hasBias,
...
// Parse one runtime flag operand type trailing the shape types. The concrete
// width/signedness is whatever the frontend produced (i1/i32/si32), so parse
// generically and let the op verifier enforce the contract.
static ParseResult parseMadFlagType(OpAsmParser &parser, bool present,
                                    Type &out) {
  if (!present) {
    return success();
  }
