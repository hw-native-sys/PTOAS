...
//===----------------------------------------------------------------------===//
// StateAccessOpInterface: acc-store copy-outs
//===----------------------------------------------------------------------===//

// The copy_matrix_cc_* family is the lowered form of the acc-store ops and
// implicitly consumes the ambient CTRL sat/atomic bits (SPR.CTRL[48]/[50]).
// The wrapper expansion only emits get_ctrl/set_ctrl around them when the
// source op carried explicit clauses, so a bare copy-out is invisible to any
// analysis that matches those ops. Declaring Consume (without the
// can-run-under-guard flag, so not a guard consumer) keeps a temporary MAD
// CTRL configuration from being installed across an acc-store.
StateResource CopyMatrixCcToGmOp::getStateResource() {
  return StateResource::Ctrl;
}

StateAccessKind CopyMatrixCcToGmOp::getStateAccessKind() {
  return StateAccessKind::Consume;
}

Value CopyMatrixCcToGmOp::getAccessedStateValue() { return {}; }

StateResource CopyMatrixCcToCbufOp::getStateResource() {
  return StateResource::Ctrl;
}

StateAccessKind CopyMatrixCcToCbufOp::getStateAccessKind() {
  return StateAccessKind::Consume;
}

Value CopyMatrixCcToCbufOp::getAccessedStateValue() { return {}; }

StateResource CopyMatrixCcToUbOp::getStateResource() {
  return StateResource::Ctrl;
}

StateAccessKind CopyMatrixCcToUbOp::getStateAccessKind() {
  return StateAccessKind::Consume;
}

Value CopyMatrixCcToUbOp::getAccessedStateValue() { return {}; }
