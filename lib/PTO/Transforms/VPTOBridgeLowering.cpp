...
private:
  static std::optional<Value> materializeBridgeCast(OpBuilder &builder,
                                                    ...
                                                    ValueRange inputs,
                                                    Location loc) {
    const bool isSingleInput = inputs.size() == 1;
    if (!isSingleInput) {
      return std::nullopt;
    }
    return builder
...
ensureWrapperDecl(ModuleOp module, BridgeLoweringState &state,
  ...
  const bool hasTypeMismatch =
      existing && existing.getFunctionType() != expected;
  if (hasTypeMismatch) {
...
static LogicalResult validateRegistryAbi(Operation *op,
                                         ...
                                         ValueRange callArgs) {
  const bool argumentCountMismatch =
      callArgs.size() != desc.arguments.size();
...
/// Validates the object-create ABI against the registry and resolves the
/// concrete size-query symbol (registry default or instance-resolved).
static FailureOr<StringRef>
validateObjectCreateAbi(BridgeObjectCreateOp op,
                        BridgeObjectCreateOp::Adaptor adaptor,
                        const BridgeFunctionDesc &desc, StringRef entryName) {
...
/// Materializes the storage for a stateful bridge object: one registry-bound
/// size query followed by an aligned stack allocation.
static FailureOr<Value>
buildObjectStorage(BridgeObjectCreateOp op, StringRef sizeSymbol,
                   const BridgeFunctionDesc &sizeDesc, uint64_t objectAlignment,
                   ModuleOp module, BridgeLoweringState &state,
                   ConversionPatternRewriter &rewriter) {
  Location loc = op.getLoc();
  Value size = rewriter
                   .create<func::CallOp>(loc, sizeSymbol, rewriter.getI64Type(),
                                         ValueRange{})
                   .getResult(0);
  ...
  return Value(rewriter.create<LLVM::AllocaOp>(
      loc, LLVM::LLVMPointerType::get(rewriter.getContext()),
      rewriter.getI8Type(), size, objectAlignment));
}
...
public:
  ...
  matchAndRewrite(BridgeObjectCreateOp op, OpAdaptor adaptor,
    ...
    FailureOr<StringRef> sizeSymbol =
        validateObjectCreateAbi(op, adaptor, *desc, entryName);
    if (failed(sizeSymbol)) {
      return failure();
    }
    const BridgeFunctionDesc *sizeDesc =
        findBridgeFunction(BridgeEntryId::PipeSize);
    ModuleOp module = op->getParentOfType<ModuleOp>();
    FailureOr<Value> storage =
        buildObjectStorage(op, *sizeSymbol, *sizeDesc, desc->objectAlignment,
                           module, state, rewriter);
    if (failed(storage)) {
      return failure();
    }
    ...
    rewriter.create<func::CallOp>(op.getLoc(), symbol, TypeRange{}, args);
...
/// Converts the op result types through the bridge type converter and
/// validates them against the registry ABI.
static FailureOr<SmallVector<Type>>
convertBridgeCallResultTypes(Operation *op, const TypeConverter *converter,
                             const BridgeFunctionDesc &desc) {
  SmallVector<Type> resultTypes;
  for (Type resultType : op->getResultTypes()) {
    Type converted = converter->convertType(resultType);
    if (!converted) {
  ...
  if (failed(validateRegistryResults(op, desc, TypeRange(resultTypes)))) {
    return failure();
  }
  return resultTypes;
}
...
public:
  ...
  matchAndRewrite(BridgeCallOp op, OpAdaptor adaptor,
    ...
    FailureOr<SmallVector<Type>> resultTypes =
        convertBridgeCallResultTypes(op, getTypeConverter(), *registryDesc);
    ...
    const bool hasNoResults = call.getNumResults() == 0;
    if (hasNoResults) {
      rewriter.eraseOp(op);
      return success();
    }
...
/// Routing policy is only used to reject operations that should have been
/// claimed by a family/typed lowering pass.
static bool findUnloweredRoutedOps(ModuleOp module,
                                   const BridgeRoutePolicy &policy) {
  ...
  module.walk([&](Operation *op) {
    ...
    StringRef family = name == "pto.tpush" || name == "pto.tpop" ||
                           ...
                           ? "pipe"
                           : "cube";
    bool routed = family == "pipe" ? policy.routesFamily("pipe")
                                   : policy.routesOp("cube", name);
    if (!routed ||
        isa<BridgeCallOp, BridgeObjectCreateOp, BridgeIntToPtrOp>(op)) {
      return;
    }
    op->emitError() << "VPTO bridge: '" << name
                    << "' is routed by policy but was not lowered into a "
                       "bridge op";
    leftoversFound = true;
  });
...
struct VPTOBridgeLoweringPass final
  ...
  void runOnOperation() override {
    ...
    bool leftoversFound = findUnloweredRoutedOps(module, *policyOr);
    if (leftoversFound) {
      signalPassFailure();
      return;
    }
