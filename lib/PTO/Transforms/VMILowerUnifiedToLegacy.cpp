...
static LogicalResult lowerVStore(VMIvStoreOp op, OpBuilder &builder) {
  ...
  if (op.getGroupAttr()) {
    builder.create<VMIGroupStoreOp>(op->getLoc(), op.getValues()[0],
  ...
  if (op.getBlockStride()) {
    ...
    if (!op.getMask().empty()) {
      ...
      auto gran = StringAttr::get(builder.getContext(), bits <= 8    ? "b8"
                                                        : bits <= 16 ? "b16"
                                                                     : "b32");
      auto maskType =
          VMIMaskType::get(builder.getContext(), valueType.getElementCount(),
                           gran, valueType.getLayout());
      auto fullLanes = builder.create<arith::ConstantOp>(
          op->getLoc(), builder.getIndexAttr(valueType.getElementCount()));
      mask = builder
                 .create<VMICreateMaskOp>(op->getLoc(), maskType,
                                             fullLanes.getResult())
                 .getResult();
    }
    Value bs = op.getBlockStride();
  ...
  StringAttr distModeAttr = op.getDistModeAttr();
  StringRef distMode = distModeAttr ? distModeAttr.getValue() : "continuous";
...
void VMILowerUnifiedToLegacyPass::runOnOperation() {
  ...
  module.walk([&](Operation *op) {
    ...
    if (isa<VMIVciOp, VMIVinterpretCastOp, VMIvSelOp, VMIVbrcOp>(op) ||
        // Category B mask logic remains legacy; vreg elementwise ops lower
        // directly to masked VPTO operations.
        (isa<VMIVandOp, VMIVorOp, VMIVxorOp>(op) &&
         isa<VMIMaskType>(op->getOperand(0).getType())) ||
        (isa<VMIVnotOp>(op) &&
         isa<VMIMaskType>(op->getOperand(0).getType())) ||
        // Category C1
  ...
  for (Operation *op : llvm::reverse(worklist)) {
    ...
    if (auto vop = dyn_cast<VMIVandOp>(op)) {
      ...
      llvm_unreachable("vreg vand must bypass unified-to-legacy lowering");
    }
    ...
    if (auto vop = dyn_cast<VMIVorOp>(op)) {
      ...
      llvm_unreachable("vreg vor must bypass unified-to-legacy lowering");
    }
    ...
    if (auto vop = dyn_cast<VMIVxorOp>(op)) {
      ...
      llvm_unreachable("vreg vxor must bypass unified-to-legacy lowering");
    }
    ...
    if (auto vop = dyn_cast<VMIVnotOp>(op)) {
      ...
      llvm_unreachable("vreg vnot must bypass unified-to-legacy lowering");
    }
  }
}
