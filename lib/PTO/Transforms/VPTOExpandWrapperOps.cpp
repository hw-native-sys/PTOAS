...
static Value getI64Constant(Location loc, RewriterBase &rewriter,
                            uint64_t value) {
...
static Value gmFieldPointer(Location loc, RewriterBase &rewriter,
...
static Value loadDevField(Location loc, RewriterBase &rewriter,
                          Value baseAddr, int64_t byteOffset,
                          Type elementType) {
...
static void storeDevField(Location loc, RewriterBase &rewriter,
...
static void storeGmField(Location loc, RewriterBase &rewriter,
                         Value baseAddr, int64_t byteOffset, Value value) {
  Value ptr =
      gmFieldPointer(loc, rewriter, baseAddr, byteOffset, value.getType());
  Value zero = rewriter.create<arith::ConstantIndexOp>(loc, 0);
...
static void ringDoorbellViaUb(Location loc, RewriterBase &rewriter,
...
static void writebackDataCache(Location loc, RewriterBase &rewriter,
                               Value addr) {
...
static Value getI32Constant(Location loc, RewriterBase &rewriter,
                            int64_t value) {
...
static Value readSessionFieldI64(Location loc, RewriterBase &rewriter,
...
static Value readSessionFieldI32(Location loc, RewriterBase &rewriter,
                                 Value session, comm_abi::SessionField field) {
...
// The struct that physically backs a session, built from the ABI's width table
// so this pass keeps no second copy of the layout.
static pto::StructType getSessionStructType(MLIRContext *ctx) {
  SmallVector<Type> fields;
  fields.reserve(comm_abi::kSessionNumFields);
  for (unsigned width : comm_abi::kSessionFieldWidths) {
    fields.push_back(IntegerType::get(ctx, width));
  }
  return pto::StructType::get(ctx, fields);
}

// Materialize the session a pto.session_init produced.
//
// A session is opaque in the IR a user writes, so the struct behind it is
// declared here, at the point the session was built. That is also the only
// place it can be declared: stack storage may originate from
// pto.declare_struct and nowhere else.
static Value materializeSession(pto::SessionInitOp op,
                                RewriterBase &rewriter) {
  Location loc = op.getLoc();
  rewriter.setInsertionPoint(op);

  pto::StructType structType = getSessionStructType(op.getContext());
  Value session = rewriter.create<pto::DeclareStructOp>(loc, structType);
...
// follow-up.
// Not a rewrite pattern: a session changes type across this expansion, from the
// opaque !pto.dma_session a caller holds to the struct that backs it, and the
// greedy rewriter cannot replace a value with one of another type. The ordered
// walk in lowerAsyncSessions drives this instead, which also fixes the order
// the two ops are handled in rather than leaving it to pattern selection.
struct SdmaGmGmExpansion {
  static LogicalResult run(pto::SdmaGmGmOp op, Value session,
                           RewriterBase &rewriter, DmaArch dmaArch) {
    Location loc = op.getLoc();
    rewriter.setInsertionPoint(op);
    ...
    if (dmaArch == DmaArch::A5 && op.getSoftPutAttr())
      return expandA5SoftPut(op, session, rewriter);

    Value recordAddr = channelRecordAddr(loc, rewriter, session, op);
    ...
    // A per-post override wins over the session default.
    Value blockBytes;
    ...
    rewriter.replaceOp(op, completionRecord(loc, rewriter, op, recordAddr));
    return success();
  }

private:
  // Base of the channel record this post drives, which is both where the queue
  // geometry is read from and what the op hands back.
  //
  //   record = contextGm + (channelIdx * channelNum) * recordBytes
  static Value channelRecordAddr(Location loc, RewriterBase &rewriter,
                                 Value session, pto::SdmaGmGmOp op) {
    ...
    return rewriter.create<arith::AddIOp>(loc, contextGm, recordOffset);
  }

  static Value completionRecord(Location loc, RewriterBase &rewriter,
                                pto::SdmaGmGmOp op, Value recordAddr) {
    return rewriter.create<pto::CastPtrOp>(loc, op.getCqRecord().getType(),
                                           recordAddr);
  }
  ...
  static LogicalResult expandA5SoftPut(pto::SdmaGmGmOp op, Value session,
                                       RewriterBase &rewriter) {
    Location loc = op.getLoc();
    MLIRContext *ctx = rewriter.getContext();
    ...
    // Nothing was posted, so there is no channel to drain and the copy is done
    // by the time this returns. The null record is what says so, and it says it
    // without reading the session's channel table -- which this path never
    // touches, and which a soft-put-only session need not even have.
    Value nullRecord = getI64Constant(loc, rewriter, 0);
    rewriter.replaceOp(op, completionRecord(loc, rewriter, op, nullRecord));
    return success();
  }

  // Write the fields a memcpy post needs. The remaining bytes of the slot keep
  // whatever the host initialized them to.
  static void writeMemcpySqe(Location loc, RewriterBase &rewriter,
...
// Lower every async session in one ordered walk: build each session, hand it to
// the posts that read it, then drop the pto.session_init ops.
//
// The order is the reason this is not left to the greedy rewriter. A post needs
// the struct its session was materialized into, so every session_init has to be
// expanded before any post reading it, and a session_init can only be erased
// once no post still refers to its result.
static LogicalResult lowerAsyncSessions(func::FuncOp func, DmaArch dmaArch) {
  SmallVector<pto::SessionInitOp> inits;
  func.walk([&](pto::SessionInitOp op) { inits.push_back(op); });
  SmallVector<pto::SdmaGmGmOp> posts;
  func.walk([&](pto::SdmaGmGmOp op) { posts.push_back(op); });
  if (inits.empty() && posts.empty()) {
    return success();
  }

  IRRewriter rewriter(func.getContext());
  DenseMap<Value, Value> sessions;
  for (pto::SessionInitOp init : inits) {
    sessions[init.getSession()] = materializeSession(init, rewriter);
  }

  for (pto::SdmaGmGmOp post : posts) {
    Value session = sessions.lookup(post.getSession());
    if (!session) {
      // Well-typed but not traceable to a session this pass materialized, so
      // there is no struct to read. The verifier cannot catch this: the type is
      // right, the value just came from somewhere else.
      return post.emitOpError()
             << "session must come from a pto.session_init in the same function";
    }
    if (failed(SdmaGmGmExpansion::run(post, session, rewriter, dmaArch))) {
      return failure();
    }
  }

  // Safe now: only a post can consume a session, and every post is gone.
  for (pto::SessionInitOp init : inits) {
...
struct VPTOExpandWrapperOpsPass
  ...
  void runOnOperation() override {
    ...
    // Ahead of the greedy patterns, because it needs a fixed order between
    // session_init and the posts that read the session it builds. What it
    // leaves behind is ordinary wrapper ops, which the patterns below then
    // expand as usual.
    if (failed(lowerAsyncSessions(func, dmaArch))) {
      signalPassFailure();
      return;
    }
