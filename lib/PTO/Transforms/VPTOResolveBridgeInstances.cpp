...
static WalkResult collectPipeLifecycle(
    BridgeObjectCreateOp create, SmallVectorImpl<BridgeCallOp> &calls,
    SmallVectorImpl<StringRef> &lifecycle) {
...
static const PipeSymbols &allocatePipeSymbols(
    const std::string &key, llvm::StringMap<PipeSymbols> &instances,
    unsigned &nextId) {
...
static WalkResult assignPipeLifecycleSymbols(
    BridgeObjectCreateOp create, ArrayRef<BridgeCallOp> calls, Attribute spec,
    const std::string &key, const PipeSymbols &symbols, MLIRContext *context) {
  create.setInstanceKeyAttr(StringAttr::get(context, key));
  create.setCalleeAttr(StringAttr::get(context, symbols.init));
  create.setSizeCalleeAttr(StringAttr::get(context, symbols.size));
  for (BridgeCallOp call : calls) {
    ...
    call.setInstanceKeyAttr(StringAttr::get(context, key));
    call.setCalleeAttr(StringAttr::get(context, symbol));
  }
  return WalkResult::advance();
}
...
static LogicalResult resolvePipeInstances(ModuleOp module) {
  ...
  WalkResult result = module.walk([&](BridgeObjectCreateOp create) {
    ...
    WalkResult collected = collectPipeLifecycle(create, calls, lifecycle);
    ...
    const PipeSymbols &symbols = allocatePipeSymbols(key, instances, nextId);
    return assignPipeLifecycleSymbols(create, calls, spec, key, symbols,
                                      module.getContext());
  });
