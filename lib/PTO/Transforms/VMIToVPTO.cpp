...
FailureOr<Value> createIotaLaneStrideChunk(Location loc, Type resultType,
                                           Value base, int64_t laneStride,
                                           int64_t laneOffset,
                                           StringAttr orderAttr,
                                           PatternRewriter &rewriter) {
  auto vregType = dyn_cast<VRegType>(resultType);
  ...
  FailureOr<Value> zero = createScalarOffsetConstant(loc, base.getType(), 0,
                                                      rewriter);
  ...
  if (isa<FloatType>(base.getType())) {
    ...
    factor = rewriter
                 .create<arith::ConstantOp>(
                     loc, rewriter.getFloatAttr(floatType, 1.0 / scale))
                 .getResult();
  } else {
  ...
  Value indices = rewriter.create<VciOp>(loc, resultType, *zero, StringAttr{})
                      .getResult();
  Value scaled = rewriter.create<VmulsOp>(loc, resultType, indices, *factor,
                                          *mask)
                     .getResult();
  StringRef order = orderAttr ? orderAttr.getValue() : StringRef("ASC");
  FailureOr<Value> chunkBase =
  ...
  if (order == "DESC") {
    Value baseVector = rewriter.create<VdupOp>(loc, resultType, *chunkBase,
                                               *mask, nullptr)
                           .getResult();
    return rewriter.create<VsubOp>(loc, resultType, baseVector, scaled, *mask)
        .getResult();
  }
  return rewriter.create<VaddsOp>(loc, resultType, scaled, *chunkBase, *mask)
      .getResult();
}
    ...
    if (layout.isContiguous() && layout.getLaneStride() != 1) {
      ...
      for (auto [index, resultType] : llvm::enumerate(resultTypes)) {
        ...
        FailureOr<Value> result = createIotaLaneStrideChunk(
            op.getLoc(), resultType, *base, laneStride,
            static_cast<int64_t>(index) * logicalLanesPerChunk,
            op.getOrderAttr(), rewriter);
        if (failed(result)) {
          return rewriter.notifyMatchFailure(
              op, "failed to materialize lane-strided iota chunk");
        }
        results.push_back(*result);
      }
      replaceOpWithFlatConvertedValues(rewriter, op, results,
