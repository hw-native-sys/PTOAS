...
// and structured SCF control flow are handled by their own analysis. Any
// state access that reaches this check is OUTSIDE a guard: scanBlockTree
...
// configuration; any op that reads or writes CTRL bits at runtime must
// declare the interface or it will be invisible to this analysis. Foreign
// dialect operations are treated conservatively. The loop-hoist restore is
// bit-scoped (only the controlled bits are restored), so an undeclared
// CTRL writer inside the loop keeps whatever it wrote in unrelated bits.
...
static void scanBlockTree(Block &block, LoopCtrlSummary &s,
  ...
  for (Operation &op : block) {
    ...
    if (auto innerFor = dyn_cast<scf::ForOp>(op)) {
      ...
      // fills the map inner-to-outer. summarizeLoop recurses through
      // scanBlockTree and may grow the map, so cache the summary locally and
      // re-find the entry instead of holding an iterator across the call.
      auto [it, inserted] = summaries.try_emplace(innerFor.getOperation());
      LoopCtrlSummary innerSummary;
      if (inserted) {
        innerSummary = summarizeLoop(innerFor, summaries);
        summaries.find(innerFor.getOperation())->second = innerSummary;
      } else {
        innerSummary = it->second;
      }
      s.ctrlClean = s.ctrlClean && innerSummary.ctrlClean;
      for (pto::CtrlStateGuardOp g : innerSummary.guards) {
        mergeGuardInto(g, s);
      }
      continue;
    }
  ...
  void process(Block *block, OpBuilder &builder) {
    ...
    for (Operation &op : llvm::make_early_inc_range(*block)) {
      ...
      if (auto access = dyn_cast<StateAccessOpInterface>(&op)) {
        // Every state access that reaches this loop is OUTSIDE a guard:
        // handleGuard unwraps a guard by moving its body ops before the
        // guard, past the early-increment iterator, so they are never
        // re-visited here. A raw MAD outside a guard therefore observes the
        // ambient state like any other access and must restore a pending
...
static void hoistOneRoot(scf::ForOp root, const LoopCtrlSummary &s,
  ...
  // Restore only the controlled bits. A whole-value set_ctrl(entry) would
  // also roll back writes to unrelated CTRL bits made inside the loop by
  // ops that do not (yet) declare StateAccessOpInterface (e.g. the mask
  // sequences later expanded from pto.tgather); keep whatever the loop left
  // in those bits and restore the entry snapshot only where this hoisted
  // configuration diverged from it.
  builder.setInsertionPointAfter(root);
  Value afterLoop = builder.create<pto::GetCtrlOp>(root.getLoc()).getResult();
  Value keepMask = builder.create<arith::ConstantIntOp>(
      root.getLoc(), static_cast<int64_t>(~s.requirement.controlled),
      mlir::pto::kValue64);
  Value entryBits = builder.create<arith::AndIOp>(
      root.getLoc(), entry,
      builder.create<arith::ConstantIntOp>(
          root.getLoc(), static_cast<int64_t>(s.requirement.controlled),
          mlir::pto::kValue64));
  Value kept = builder.create<arith::AndIOp>(root.getLoc(), afterLoop, keepMask);
  Value restored =
      builder.create<arith::OrIOp>(root.getLoc(), kept, entryBits);
  builder.create<pto::SetCtrlOp>(root.getLoc(), restored);
}
...
static void hoistLoopConfigurations(
  ...
  for (scf::ForOp root : preOrder) {
    ...
    // An earlier (outer) hoist may already have unwrapped this loop's
    // guards; check membership BEFORE the eligibility query, which
    // dereferences the guard operations and would be use-after-free on
    // erased ones.
    ...
    if (!loopHoistEligible(root, it->second, summaries)) {
      continue;
    }
