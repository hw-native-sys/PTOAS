...
getTPipePipeTypeToken(int8_t dirMask, PTOArch targetArch) {
...
getTPipeSyncTypeTokens(const std::string &pipeTypeToken) {
  if (pipeTypeToken == "FIFOType::GM_FIFO")
    return {"TSyncOpType::TSTORE_C2GM_UFOFF", "TSyncOpType::TLOAD"};
  if (pipeTypeToken == "FIFOType::MAT_FIFO")
...
struct PTOInitializePipeToEmitC
  ...
  LogicalResult matchAndRewrite(mlir::pto::InitializePipeOp op, OpAdaptor adaptor,
    ...
    auto pipeTypeToken = getTPipePipeTypeToken(dirMask, targetArch);
    ...
    Value pipeHandle;
    if (*pipeTypeToken == "FIFOType::GM_FIFO") {
      pipeHandle = peelUnrealized(adaptor.getGmSlotBuffer());
    } else if (*pipeTypeToken == "FIFOType::VEC_FIFO") {
      pipeHandle = peelUnrealized(adaptor.getC2vConsumerBuf());
    } else {
      pipeHandle = peelUnrealized(adaptor.getV2cConsumerBuf());
    }

    auto emitPipeTy = cast<Type>(getTypeConverter()->convertType(op.getPipe().getType()));
    rewriter.replaceOpWithNewOp<emitc::CallOpaqueOp>(
        op, TypeRange{emitPipeTy}, tpipeTok,
        /*args=*/ArrayAttr{},
        /*templateArgs=*/ArrayAttr{},
        /*operands=*/ValueRange{pipeHandle});
    return success();
  }
