...
buildSpecialScaleGlobalTensorTypeSpec(ArrayRef<int64_t> shape,
                                      StringRef elemTypeStr,
                                      pto::BLayout blayout,
                                      pto::SLayout slayout) {
  if (blayout == pto::BLayout::RowMajor &&
    ...
    return SpecialGlobalTensorTypeSpec{
        "TileShape2D<" + elemTypeStr.str() + ", " +
            std::to_string(shape[0]) + ", " + std::to_string(shape[1]) +
            ", pto::Layout::MX_A_ZZ>",
        "BaseShape2D<" + elemTypeStr.str() + ", " +
            std::to_string(shape[0]) + ", " + std::to_string(shape[1]) +
            ", pto::Layout::MX_A_ZZ>",
        "pto::Layout::MX_A_ZZ",
    };
...
getSpecialScaleGlobalTensorTypeSpecForTileValue(Value dstValue,
  ...
  return buildSpecialScaleGlobalTensorTypeSpec(
      effectiveShape, getEmitCScalarTypeToken(elemTy), blayout, slayout);
}
...
private:
  void registerScalarConversions(MLIRContext *Ctx) {
    ...
    registerIndexAndVectorConversions(Ctx);
  }
  ...
  void registerIndexAndVectorConversions(MLIRContext *Ctx) {
  ...
  void registerPTOConversions(MLIRContext *Ctx) {
    registerPTOPointerAndAggregateConversions(Ctx);
    registerPTOAsyncAndViewConversions(Ctx);
  }

  void registerPTOPointerAndAggregateConversions(MLIRContext *Ctx) {
    // ---------------------------------------------------------
    // 2. PTO 特殊类型 (透传或转换)
    // ---------------------------------------------------------
  ...
  void registerPTOAsyncAndViewConversions(MLIRContext *Ctx) {
...
static bool isMatchingPeerFixpipeConsumerInit(
    Operation *candidate, func::FuncOp peerFunc,
    FlatSymbolRefAttr ownerFuncAttr, StringAttr reserveNameAttr,
    IntegerAttr dirMaskAttr) {
  if (!isa<InitializeL2LPipeOp, InitializeL2G2LPipeOp>(candidate) ||
      !getPipeInitAccPushEpilogue(candidate)) {
    return false;
  }
  ...
  return candidateOwnerFuncAttr && candidateReserveNameAttr &&
         candidateDirMaskAttr && candidateOwnerFuncAttr == ownerFuncAttr &&
         candidateReserveNameAttr == reserveNameAttr &&
         candidateDirMaskAttr.getInt() == dirMaskAttr.getInt() &&
         candidateFunc == peerFunc;
}
...
static FailureOr<Operation *> findPeerFixpipeConsumerInit(Operation *producerInit) {
  ...
  peerFunc.walk([&matchedInit, &matchedInitCount, ownerFuncAttr,
    ...
    if (!isMatchingPeerFixpipeConsumerInit(candidate, peerFunc, ownerFuncAttr,
                                            reserveNameAttr, dirMaskAttr)) {
...
static LogicalResult rematerializeFixpipePush(
    TPushOp tpush, llvm::DenseMap<int32_t, SetQuantScalarOp> &activeScalarById,
...
static LogicalResult rematerializeFixpipeQuantBindingsInRegions(
    Operation *op, SmallVectorImpl<Operation *> &eraseList) {
...
static LogicalResult rematerializeFixpipeQuantBindingsInBlock(
  ...
  for (Operation *op : originalOps) {
    if (auto setQuantScalar = dyn_cast<SetQuantScalarOp>(op)) {
      ...
      if (failed(rematerializeFixpipePush(tpush, activeScalarById,
                                          activeVectorById))) {
        return failure();
      }
    }

    if (failed(rematerializeFixpipeQuantBindingsInRegions(op, eraseList))) {
      return failure();
    }
  }
  return success();
...
static Value buildGlobalTensorFromMemref(ConversionPatternRewriter &rewriter,
  ...
  emitGlobalTensorTypeAliases(rewriter, loc, names, anchor, mrTy, shape,
                              strides, shape5D, stride5D, basePtr);
...
struct PTOGetScaleAddrToEmitC
  ...
  static std::optional<SourceInfo> getSourceInfo(pto::TGetScaleAddrOp op) {
    Type sourceType = op.getSrc().getType();
    Type elementType;
    Attribute memorySpace;
    if (auto srcTy = dyn_cast<MemRefType>(sourceType)) {
      elementType = srcTy.getElementType();
      memorySpace = srcTy.getMemorySpace();
    } else if (auto srcTy = dyn_cast<pto::TileBufType>(sourceType)) {
      elementType = srcTy.getElementType();
      memorySpace = srcTy.getMemorySpace();
    }

    auto addressSpace = dyn_cast_or_null<pto::AddressSpaceAttr>(memorySpace);
    if (!addressSpace || !elementType) {
      return std::nullopt;
    }
    return SourceInfo{addressSpace.getAddressSpace(),
                      getEmitCScalarTypeToken(elementType)};
  }

  static bool isEmitCTileLike(Type type) {
    auto opaqueType = dyn_cast<emitc::OpaqueType>(type);
    return opaqueType &&
           (opaqueType.getValue().contains("Tile<") ||
            opaqueType.getValue().contains("ConvTile<"));
  }
  ...
  LogicalResult matchAndRewrite(pto::TGetScaleAddrOp op, OpAdaptor adaptor,
    ...
    auto sourceInfo = getSourceInfo(op);
    if (!sourceInfo) {
      return rewriter.notifyMatchFailure(
    ...
    Value rawPtr = src;
    rawPtr = materializeTileDataValue(rewriter, loc, src,
                                      sourceInfo->addressSpace,
                                      sourceInfo->elementType);
    if (tileDataReturnsIntegralAddress(sourceInfo->addressSpace)) {
      rawPtr = materializeAddressAsPointer(
          rewriter, loc, rawPtr, sourceInfo->addressSpace,
          sourceInfo->elementType);
    }
...
struct RuntimeGlobalTensorShapeStrideValues {
  SmallVector<Value, 5> shape;
  SmallVector<Value, 5> stride;
};

static RuntimeGlobalTensorShapeStrideValues
buildRuntimeGlobalTensorShapeStrideValues(
    ConversionPatternRewriter &rewriter, Location loc,
  ...
  RuntimeGlobalTensorShapeStrideValues result;
  for (int64_t dim = 0; dim < shift; ++dim) {
    result.shape.push_back(makeViewIndexConstant(rewriter, loc, 1));
  }
  for (Value value : runtimeShape) {
    result.shape.push_back(castViewIndexToEmitC(rewriter, loc, value));
  }

  result.stride.resize(5);
  for (auto [index, value] : llvm::enumerate(runtimeStrides)) {
    result.stride[shift + static_cast<int64_t>(index)] =
        castViewIndexToEmitC(rewriter, loc, value);
  }
  if (shift == 5) {
    for (int64_t dim = 0; dim < 5; ++dim) {
      result.stride[dim] = makeViewIndexConstant(rewriter, loc, 1);
    }
  } else {
    for (int64_t dim = shift - 1; dim >= 0; --dim) {
      result.stride[dim] =
          rewriter
              .create<emitc::MulOp>(loc, result.stride[dim + 1].getType(),
                                     result.shape[dim + 1],
                                     result.stride[dim + 1])
              .getResult();
    }
...
buildRuntimeGlobalTensorShapeStride(ConversionPatternRewriter &rewriter,
  ...
  RuntimeGlobalTensorShapeStrideValues values =
      buildRuntimeGlobalTensorShapeStrideValues(rewriter, loc, staticShape,
                                                runtimeShape, runtimeStrides);
  ...
  result.shape =
      rewriter
          .create<emitc::CallOpaqueOp>(
              ...
              result.shapeType, ArrayAttr{}, ArrayAttr{}, values.shape)
          .getResult(0);
  result.stride =
      rewriter
          .create<emitc::CallOpaqueOp>(
              loc, emitc::OpaqueType::get(ctx, result.strideType),
              result.strideType, ArrayAttr{}, ArrayAttr{}, values.stride)
          .getResult(0);
  return result;
}
...
struct SCFWhileToCF : public OpRewritePattern<scf::WhileOp> {
  ...
  static SmallVector<Block *> collectWhileRegionBlocks(Region &region) {
  ...
  static LogicalResult replaceWhileCondition(
      PatternRewriter &rewriter, scf::WhileOp op, Location loc,
      ArrayRef<Block *> beforeBlocks, Block *afterEntry,
  ...
  static LogicalResult replaceWhileYields(PatternRewriter &rewriter,
                                          scf::WhileOp op, Location loc,
                                          ArrayRef<Block *> afterBlocks,
                                          Block *beforeEntry) {
  ...
  static void inlineWhileRegions(PatternRewriter &rewriter, scf::WhileOp op,
                                 Block *afterWhileBlock) {
  ...
  LogicalResult matchAndRewrite(scf::WhileOp op,
    ...
    SmallVector<Block *> beforeBlocks = collectWhileRegionBlocks(op.getBefore());
    SmallVector<Block *> afterBlocks = collectWhileRegionBlocks(op.getAfter());
    ...
    inlineWhileRegions(rewriter, op, afterWhileBlock);
    ...
    if (failed(replaceWhileCondition(rewriter, op, loc, beforeBlocks, afterEntry,
                                     afterWhileBlock))) {
      return failure();
    }
    if (failed(replaceWhileYields(rewriter, op, loc, afterBlocks, beforeEntry))) {
      return failure();
    }
...
static void populatePTOToEmitCSynchronizationPatterns(
...
static void populatePTOToEmitCSyncAndVectorPatterns(
    RewritePatternSet &patterns, TypeConverter &typeConverter, MLIRContext *ctx,
    PTOArch targetArch) {
  populatePTOToEmitCSynchronizationPatterns(patterns, typeConverter, ctx);
...
static void populatePTOToEmitCTileScalarPatterns(
    RewritePatternSet &patterns, TypeConverter &typeConverter, MLIRContext *ctx) {
...
static void populatePTOToEmitCScalarAndArithmeticPatterns(
    RewritePatternSet &patterns, TypeConverter &typeConverter, MLIRContext *ctx,
    PTOArch targetArch) {
  populatePTOToEmitCTileScalarPatterns(patterns, typeConverter, ctx);
...
static void populatePTOToEmitCIntegerPatterns(
    RewritePatternSet &patterns, TypeConverter &typeConverter, MLIRContext *ctx) {
...
static void populatePTOToEmitCIntegerAndFloatPatterns(
    RewritePatternSet &patterns, TypeConverter &typeConverter, MLIRContext *ctx,
    PTOArch targetArch) {
  populatePTOToEmitCIntegerPatterns(patterns, typeConverter, ctx);
...
static void populatePTOToEmitCTileMemoryPatterns(
    RewritePatternSet &patterns, TypeConverter &typeConverter, MLIRContext *ctx) {
...
static void populatePTOToEmitCCommunicationPatterns(
    RewritePatternSet &patterns, TypeConverter &typeConverter, MLIRContext *ctx,
    PTOArch targetArch) {
...
static void populatePTOToEmitCDataObjectPatterns(
    RewritePatternSet &patterns, TypeConverter &typeConverter, MLIRContext *ctx) {
...
static void populatePTOToEmitCSynchronizationAndUtilityPatterns(
    RewritePatternSet &patterns, TypeConverter &typeConverter, MLIRContext *ctx,
    PTOArch targetArch) {
...
static void populatePTOToEmitCControlFlowPatterns(
    RewritePatternSet &patterns, TypeConverter &typeConverter, MLIRContext *ctx) {
...
static void populatePTOToEmitCMemoryAndCommunicationPatterns(
    RewritePatternSet &patterns, TypeConverter &typeConverter, MLIRContext *ctx,
    PTOArch targetArch) {
  populatePTOToEmitCTileMemoryPatterns(patterns, typeConverter, ctx);
  populatePTOToEmitCCommunicationPatterns(patterns, typeConverter, ctx,
                                           targetArch);
  populatePTOToEmitCDataObjectPatterns(patterns, typeConverter, ctx);
  populatePTOToEmitCSynchronizationAndUtilityPatterns(patterns, typeConverter,
                                                       ctx, targetArch);
