...
# ── MX quantize (ND/DN templates + helpers) ──

def _mx_nd_2d_tiles(operand_memory_spaces, grp_axis=None, **_):
    """ND (flat + Exp2D auto-dispatch) template constraint; rejects DN (grp_axis=0/axis0)."""
...
def template_tquant_mxfp8_f32(src: pto.Tile, dst: pto.Tile,
    ...
    """MXFP8: f32 → fp8 E4M3.
    exp.rows > 1 selects the 2D Packed path, otherwise flat."""
...
def template_tquant_mxfp8_b16(src: pto.Tile, dst: pto.Tile, exp: pto.Tile, max_tile: pto.Tile, scaling: pto.Tile):
    ...
    exp.rows > 1 selects the 2D Packed path (B16_2D overload 1), otherwise flat."""
    scale_alg = pto.get_op_attr("quant_scale_alg", "ocp")
...
def _mx_dn_tiles(operand_memory_spaces, grp_axis=None, **_):
    """DN template constraint; only accepts explicit DN (grp_axis=0/axis0)."""
    if not all(space in {"ub", "vec"} for space in operand_memory_spaces):
        return False
        ...
        for j in range(num_grps_per_col):
            ...
            with pto.for_(0, grp_size, step=1) as k:
                ...
                v_fp8 = pto.vcvt(v_input, pto.f8e4m3, preg_b32,
                                 rnd=pto.VcvtRoundMode.R, sat=pto.VcvtSatMode.SAT,
                                 part=pto.VcvtPartMode.P0)
                # PK4_B32 packs 4 fp8 bytes per 32-bit lane; the store
                # contract requires a mask<b32>, so the b32 lane mask is
                # reused.
                pto.vsts(v_fp8, dst_ptr, dst_off, preg_b32,
    ...
    for i in range(num_vls_per_row):
        ...
        for j in range(num_grps_per_col):
            ...
            with pto.for_(0, grp_size, step=1) as k:
                ...
                # PK4_B32 requires mask<b32> (dist-contract table); b16 lane
                # count equals the packed 32-bit group count here.
                preg_b32_f4, _ = pto.make_mask(pto.f32, valid_cols // 2)
                pto.vsts(v_fp4, dst_ptr, off // 2, preg_b32_f4,
                         dist=pto.VStoreDist.PK4_B32)
...
def _calc_quantized_fp4e2m1_values_bf16(src_ptr, scaling_ptr, dst_ptr, num_groups, lanes_b16):
    ...
    for window in range(window_count):
        ...
        v_output_1 = pto.vcvt(v_intlv_1, pto.f4e2m1x2, preg_b16_window,
                              rnd=pto.VcvtRoundMode.R, part=pto.VcvtPartMode.P0)
        # PK4_B32 requires mask<b32> (dist-contract table); the mask scalar
        # counts packed BYTES (mirrors the passing fp8 path at the f32 DN
        # store which masks with the element/byte count).
        preg_b32_out, _ = pto.make_mask(pto.f32, k_packed_bytes_per_half_window)
        pto.vsts(v_output_0, dst_ptr, window * k_packed_bytes_per_window, preg_b32_out,
                 dist=pto.VStoreDist.PK4_B32)
        pto.vsts(v_output_1, dst_ptr, window * k_packed_bytes_per_window + k_packed_bytes_per_half_window,
                 preg_b32_out, dist=pto.VStoreDist.PK4_B32)
