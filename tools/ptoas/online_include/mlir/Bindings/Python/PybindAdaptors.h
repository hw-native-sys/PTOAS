...
//===----------------------------------------------------------------------===//
//
// LOCAL MODIFICATION (PTOAS online-build closure): the upstream
// `#include "llvm/ADT/Twine.h"` and its four `llvm::Twine(...).str()` call
// sites have been replaced with plain `std::string` concatenation so this
// header can be compiled online with only the shipped `mlir-c/*` + `pto-c/*`
// closure (no LLVM C++ ADT tree). Behavior is unchanged.
//===----------------------------------------------------------------------===//
...
static py::object mlirApiObjectToCapsule(py::handle apiObject) {
  ...
  if (!py::hasattr(apiObject, MLIR_PYTHON_CAPI_PTR_ATTR)) {
    ...
    throw py::type_error(
        std::string("Expected an MLIR object (got ") + repr + ").");
  }
...
protected:
  /// Register the ``get_static_typeid`` method and the cast-from-typeid caster
  /// used by both attribute/type subclasses when the dialect provides a typeID.
  void registerTypeIdCaster(MlirTypeID (*getTypeIDFunction)()) {
    ...
    py::module::import(MAKE_MLIR_PYTHON_QUALNAME("ir"))
        .attr(MLIR_PYTHON_CAPI_TYPE_CASTER_REGISTER_ATTR)(
            ...
            [thisClass = thisClass](const py::object &mlirObject) {
              return thisClass(mlirObject);
            }));
  }
...
public:
  ...
  mlir_attribute_subclass(py::handle scope, const char *typeClassName,
        ...
        [superCls, isaFunction, captureTypeName](py::object cls,
          ...
          if (!isaFunction(rawAttribute)) {
            ...
            throw std::invalid_argument(
                (std::string("Cannot cast attribute to ") + captureTypeName +
                 " (from " + origRepr + ")"));
          }
    ...
    if (getTypeIDFunction) {
      registerTypeIdCaster(getTypeIDFunction);
    }
  }
};
    ...
    py::cpp_function newCf(
        [superCls, isaFunction, captureTypeName](py::object cls,
          ...
          if (!isaFunction(rawType)) {
            ...
            throw std::invalid_argument((std::string("Cannot cast type to ") +
    ...
    py::cpp_function newCf(
        [superCls, isaFunction, captureValueName](py::object cls,
          ...
          if (!isaFunction(rawValue)) {
            ...
            throw std::invalid_argument((std::string("Cannot cast value to ") +
...
public:
  ...
  ~CollectDiagnosticsToStringScope() {
    if (!errorMessage.empty()) {
      std::fprintf(stderr, "unchecked error message: %s\n", errorMessage.c_str());
    }
