...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/trems.
#
# A5 legacy trems integer cases are intentionally not included: the legacy
# suite skips them because the vector divide used by the implementation does
# not support integer element types.
...
def _make_kernel(name, dtype, shape):
    ...
    @pto.jit(name="trems_" + name, target="a5")
    def _kernel(
        ...
        pto.tile.rems(src_tile, 3.0, dst_tile)
...
def _make_expected(src):
    ...
    return (src - np.trunc(src / scalar) * scalar).astype(
        src.dtype, copy=False
    )
