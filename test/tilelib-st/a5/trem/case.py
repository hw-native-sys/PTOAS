...
# PTODSL rewrite of test/tilelang_st/npu/a5/src/st/testcase/trem.
#
# The public tile.rem wrapper supplies the legacy temporary operand when it is
# omitted. Inputs cover signed operands, so the golden verifies floor
# remainder semantics, whose result follows the divisor's sign.
...
def _make_kernel(name, dtype, shape, precision):
    ...
    @pto.jit(name="trem_" + name, target="a5")
    def _kernel(
        ...
        pto.tile.rem(src0_tile, src1_tile, dst_tile, precision=precision)
...
def _make_expected(src0, src1):
    return np.remainder(src0, src1).astype(src0.dtype, copy=False)
