...
"""Build and replay a frozen A/P0/B matrix on one exclusively queued A5."""
import argparse
import hashlib
import json
import os
...
def correctness_order(manifest):
    for case in manifest["cases"]:
        ...
        forward = [variants[name] for name in (("B", "P0", "A") if "B" in variants else ("P0", "A"))]
...
def performance_order(manifest):
    for case in manifest["cases"]:
        ...
        for block in range(20):
            ...
            for name in order:
                yield case, variants[name], block % 3, block, "paired_profile", "".join(order), name
        for repeat in range(5):
            yield case, variants["P0"], repeat % 3, repeat, "p0_profile", None, "P0"
...
def run_one(args, case, variant, seed, repeat, stage, order, role):
    ...
    label = f"{stage}-seed{seed}-r{repeat}-{name}"
...
def main():
    ...
    if provenance["runtime_tools"]["run_ab.py"] != sha(Path(__file__)):
        raise ValueError("matrix requires its frozen A/B runner")
    if args.mode == "build":
