...
def build():
    with Context() as ctx:
        ...
        with Location.unknown(ctx):
            ...
            src_partition_view = pto.PartitionTensorViewType.get([16, 128], f32, ctx)
            dst_partition_view = pto.PartitionTensorViewType.get([16, 64], f32, ctx)
            ...
            src_tile_type = pto.TileBufType.get([16, 128], f32, vec, [16, 128], config, ctx)
            dst_tile_type = pto.TileBufType.get([16, 64], f32, vec, [16, 64], config, ctx)

            function_type = func.FunctionType.get([ptr_f32] * 3, [])
            with InsertionPoint(module.body):
                function = func.FuncOp("tdeinterleave_single_runtime_kernel", function_type)
            ...
            with InsertionPoint(entry):
                ...
                src_ptr, dst0_ptr, dst1_ptr = entry.arguments

                src_view = pto.MakeTensorViewOp(
                    tensor_view, src_ptr, [c16, c128], [c128, c1]
                ).result
                ...
                src_partition = pto.PartitionViewOp(
                    src_partition_view, src_view, offsets=[c0, c0], sizes=[c16, c128]
                ).result
                dst0_partition = pto.PartitionViewOp(
                    dst_partition_view, dst0_view, offsets=[c0, c0], sizes=[c16, c64]
                ).result
                dst1_partition = pto.PartitionViewOp(
                    dst_partition_view, dst1_view, offsets=[c0, c0], sizes=[c16, c64]
                ).result

                src_tile = pto.AllocTileOp(src_tile_type).result
                dst0_tile = pto.AllocTileOp(dst_tile_type).result
                dst1_tile = pto.AllocTileOp(dst_tile_type).result
                pto.TLoadOp(None, src_partition, src_tile)
                pto.TDeInterleaveOp([src_tile], [dst0_tile, dst1_tile])
