...
// End-to-end host driver for the cast-spine case "roundtrip-ui32-ui8-ui32".
//
...
void LaunchVmi_cast_spine_ui32_ui8_ui32_kernel(uint32_t *src, uint32_t *dst, void *stream);

int main() {
  constexpr size_t kElems = 4096;
  // ReadFile() takes a non-const size; keep the byte count in a mutable local.
  size_t kBytes = kElems * sizeof(uint32_t);
  ...
  for (int i = 0; i < warmup; ++i) {
    LaunchVmi_cast_spine_ui32_ui8_ui32_kernel(srcDevice, dstDevice, stream);
  }
  ...
  ACL_CHECK(aclrtSynchronizeStream(stream));
  {
    ...
    std::printf("[TIMING] case=roundtrip-ui32-ui8-ui32 iters=%d warmup=%d total_ms=%.6f "
                "per_launch_us=%.6f\n",
