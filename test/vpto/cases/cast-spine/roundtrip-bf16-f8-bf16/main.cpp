...
// End-to-end host driver for the cast-spine case "roundtrip-bf16-f8-bf16".
//
...
void LaunchVmi_cast_spine_bf16_f8_bf16_kernel(uint16_t *src, uint16_t *dst, void *stream);
...
int main() {
  ...
  for (int i = 0; i < warmup; ++i) {
    LaunchVmi_cast_spine_bf16_f8_bf16_kernel(srcDevice, dstDevice, stream);
  }
  ...
  ACL_CHECK(aclrtSynchronizeStream(stream));
  {
    ...
    std::printf("[TIMING] case=roundtrip-bf16-f8-bf16 iters=%d warmup=%d total_ms=%.6f "
                "per_launch_us=%.6f\n",
