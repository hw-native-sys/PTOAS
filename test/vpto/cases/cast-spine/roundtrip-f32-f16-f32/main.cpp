...
// End-to-end host driver for the cast-spine case "roundtrip-f32-f16-f32".
//
// The kernel converts 8192 elements per launch.  Two environment
...
void LaunchVmi_cast_spine_f32_f16_f32_kernel(float *src, float *dst, void *stream);

int main() {
  constexpr size_t kElems = 8192;
  // ReadFile() takes a non-const size; keep the byte count in a mutable local.
  size_t kBytes = kElems * sizeof(float);
  ...
  for (int i = 0; i < warmup; ++i) {
    LaunchVmi_cast_spine_f32_f16_f32_kernel(srcDevice, dstDevice, stream);
  }
  ...
  ACL_CHECK(aclrtSynchronizeStream(stream));
  {
    ...
    std::printf("[TIMING] case=roundtrip-f32-f16-f32 iters=%d warmup=%d total_ms=%.6f "
                "per_launch_us=%.6f\n",
