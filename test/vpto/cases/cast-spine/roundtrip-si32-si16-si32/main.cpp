...
// End-to-end host driver for the cast-spine case "roundtrip-si32-si16-si32".
//
// The kernel converts 4096 elements per launch.  Two environment
...
void LaunchVmi_cast_spine_si32_si16_si32_kernel(int32_t *src, int32_t *dst, void *stream);

int main() {
  constexpr size_t kElems = 4096;
  // ReadFile() takes a non-const size; keep the byte count in a mutable local.
  size_t kBytes = kElems * sizeof(int32_t);
  ...
  for (int i = 0; i < warmup; ++i) {
    LaunchVmi_cast_spine_si32_si16_si32_kernel(srcDevice, dstDevice, stream);
  }
  ...
  ACL_CHECK(aclrtSynchronizeStream(stream));
  {
    ...
    std::printf("[TIMING] case=roundtrip-si32-si16-si32 iters=%d warmup=%d total_ms=%.6f "
                "per_launch_us=%.6f\n",
