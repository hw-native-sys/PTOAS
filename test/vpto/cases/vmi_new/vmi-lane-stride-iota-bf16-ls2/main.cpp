...
// bf16 is stored as uint16_t (same width, no native C++ bf16 type needed here).
void LaunchVmi_lane_stride_iota_bf16_ls2_kernel(uint16_t *dst, void *stream);

int main() {
  // 64 logical bf16 lanes: vci(0.0, lane_stride=2) → [0.0..63.0] as bf16.
  constexpr size_t kDstElems = 64;
  ...
  LaunchVmi_lane_stride_iota_bf16_ls2_kernel(dstDevice, stream);
