...
void LaunchVmi_quant_bf16x2_to_f4x2_tail_kernel(uint16_t *src, uint8_t *dst,
                                                void *stream);

int main() {
  constexpr size_t kSrcElems = 1024;  // bf16 source buffer
  constexpr size_t kDstElems = 512;   // f4x2 output buffer
  ...
  LaunchVmi_quant_bf16x2_to_f4x2_tail_kernel(srcDevice, dstDevice, stream);
