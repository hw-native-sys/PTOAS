...
void LaunchVmi_quant_bf16x2_to_f4x2_overflow_kernel(uint16_t *src, uint8_t *dst,
                                                    void *stream);

int main() {
  constexpr size_t kSrcElems = 128;   // bf16
  constexpr size_t kDstElems = 64;    // f4x2 (1 byte per pair)
  ...
  LaunchVmi_quant_bf16x2_to_f4x2_overflow_kernel(srcDevice, dstDevice, stream);
