...
void LaunchVmi_dequant_f4x2_to_bf16x2_tail_kernel(uint8_t *src, uint16_t *dst,
                                                  void *stream);

int main() {
  constexpr size_t kSrcElems = 256;   // f4x2 bytes in the UB buffer
  constexpr size_t kDstElems = 512;   // bf16 output lanes in the UB buffer
  size_t srcBytes = kSrcElems * sizeof(uint8_t);
  size_t dstBytes = kDstElems * sizeof(uint16_t);
  ...
  LaunchVmi_dequant_f4x2_to_bf16x2_tail_kernel(srcDevice, dstDevice, stream);
