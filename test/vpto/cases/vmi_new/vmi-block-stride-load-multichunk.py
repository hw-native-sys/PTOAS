...
"""block 形态（块步长）`pto.vmi.vload`：payload 超过一个物理 carrier（多 carrier `vsldb`）。

覆盖 pto-as 单提交 `c26431493` 的 §1.3（多 carrier stride 访存）：
同一个 block 形态，i32 / vl=256 的 payload = 1024B = 4 个 carrier，
改动前报 `VMI-UNSUPPORTED: pto.vmi.stride_load lowers through pto.vsldb only for one
contiguous physical result/mask chunk`。形态取自 pto_test
...
BLOCK_STRIDE = 8
ELEM_BYTES = 4
EV = 32 // ELEM_BYTES  # i32 -> 8
NBLK = max(1, (VL + EV - 1) // EV)  # 32
SRC_ELEMS = (NBLK - 1) * BLOCK_STRIDE * EV + EV  # 1992

SRC = """module attributes {pto.target_arch = "a5", pto.kernel_kind = #pto.kernel_kind<vector>} {
  func.func @vmi_block_stride_load_multichunk_kernel(%src_gm: !pto.ptr<i32, gm>,
    ...
    %ub_dst = pto.castptr %c8192_i64 : i64 -> !pto.ptr<i32, ub>

    pto.mte_gm_ub %src_gm, %ub_src, %c0_i64, %c7968_i64
...
def vmi_block_stride_load_multichunk_kernel(
    src_gm: pto.ptr(pto.i32, "gm"),
    dst_gm: pto.ptr(pto.i32, "gm"),
):
    pass
...
CASES = [
    golden_output_case(
        "vmi_block_stride_load_multichunk",
        vmi_block_stride_load_multichunk_kernel,
        inputs=make_inputs,
