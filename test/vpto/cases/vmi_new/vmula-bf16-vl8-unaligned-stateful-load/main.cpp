...
void LaunchVmi_vmula_bf16_vl8_unaligned_kernel(int16_t *acc, int16_t *lhs, int16_t *rhs,
                                     int16_t *dst, void *stream);
...
int main() {
  ...
  ACL_CHECK(aclrtMemcpy(accDevice, kStagingBytes, accHost, kStagingBytes,
                         ACL_MEMCPY_HOST_TO_DEVICE));
  ACL_CHECK(aclrtMemcpy(lhsDevice, kStagingBytes, lhsHost, kStagingBytes,
                         ACL_MEMCPY_HOST_TO_DEVICE));
  ACL_CHECK(aclrtMemcpy(rhsDevice, kStagingBytes, rhsHost, kStagingBytes,
                         ACL_MEMCPY_HOST_TO_DEVICE));
  ACL_CHECK(aclrtMemcpy(dstDevice, kOutputBytes, dstHost, kOutputBytes,
                         ACL_MEMCPY_HOST_TO_DEVICE));
  LaunchVmi_vmula_bf16_vl8_unaligned_kernel(accDevice, lhsDevice, rhsDevice, dstDevice, stream);
  ACL_CHECK(aclrtSynchronizeStream(stream));
  ACL_CHECK(aclrtMemcpy(dstHost, kOutputBytes, dstDevice, kOutputBytes,
