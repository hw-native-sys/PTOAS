...
void LaunchVmi_short_extend_store_i8_kernel(int8_t *src, int32_t *dst,
                                            void *stream);

int main() {
  constexpr size_t kSrcElems = 256;
  constexpr size_t kDstElems = 256;
  size_t srcBytes = kSrcElems * sizeof(int8_t);
  size_t dstBytes = kDstElems * sizeof(int32_t);
  int8_t *srcHost = nullptr;
  int8_t *srcDevice = nullptr;
  ...
  LaunchVmi_short_extend_store_i8_kernel(srcDevice, dstDevice, stream);
