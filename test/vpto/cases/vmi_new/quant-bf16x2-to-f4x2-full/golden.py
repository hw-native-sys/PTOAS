...
# Reference for bf16x2 -> f4E1M2x2 (full 256-lane, bf16x2 direct-load path).
# Same quantization as the contiguous case; the source is loaded as 256 bf16x2
# (= 512 bf16) and each pair produces one f4E1M2x2 byte.
...
SRC_ELEMS = 512          # bf16 input lanes (256 bf16x2 pairs)
DST_ELEMS = 256          # f4x2 output bytes
...
# Period 15 is coprime with the 64/128-lane physical partitions. This prevents
# a part/chunk swap in the d4 lowering from being hidden by a short repetition.
VALUES = np.array([
...
def generate(output_dir: Path) -> None:
    ...
    f4 = quantize_f4e1m2(bf16_to_f32(src_bf16)).astype(np.int32)  # [512]
    even = f4[0::2]
    odd = f4[1::2]
    if PAIR_HI_FIRST:
