...
void LaunchVmi_quant_bf16x2_to_f4x2_contiguous_kernel(uint16_t *src, uint8_t *dst,
                                                      void *stream);

int main() {
  constexpr size_t kSrcElems = 256;   // bf16
  constexpr size_t kDstElems = 128;   // f4x2 (1 byte per pair)
  ...
  LaunchVmi_quant_bf16x2_to_f4x2_contiguous_kernel(srcDevice, dstDevice, stream);
