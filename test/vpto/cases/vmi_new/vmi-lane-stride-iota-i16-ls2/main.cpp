...
void LaunchVmi_lane_stride_iota_i16_ls2_kernel(int16_t *dst, void *stream);

int main() {
  // 64 logical i16 lanes: vci(0, lane_stride=2) → [0..63].
  constexpr size_t kDstElems = 64;
  size_t dstBytes = kDstElems * sizeof(int16_t);
  ...
  LaunchVmi_lane_stride_iota_i16_ls2_kernel(dstDevice, stream);
