...
void LaunchVmi_lane_stride_iota_i32_ls2_kernel(int32_t *src, int32_t *dst,
                                               void *stream);

int main() {
  // src: 32 i32 zeros (seeds lane_stride=2 via vload).
  // dst: 32 i32 iota [0..31].
  constexpr size_t kElems = 32;
  size_t srcBytes = kElems * sizeof(int32_t);
  ...
  LaunchVmi_lane_stride_iota_i32_ls2_kernel(srcDevice, dstDevice, stream);
