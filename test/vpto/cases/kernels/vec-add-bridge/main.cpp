...
void LaunchVec_add_bridge_kernel(float *a, float *b, float *c, void *stream);

int main() {
  constexpr size_t kElem = 8 * 16;
  constexpr size_t aSize = kElem * sizeof(float);
  constexpr size_t bSize = kElem * sizeof(float);
  constexpr size_t cSize = kElem * sizeof(float);
  ...
  LaunchVec_add_bridge_kernel(aDevice, bDevice, cDevice, stream);
