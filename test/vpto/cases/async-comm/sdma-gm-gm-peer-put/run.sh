#!/bin/bash
# Build and run the cross-device GM->peer GM SDMA case.
#
...
"${BISHENG_BIN}" \
  ...
  -shared -Wl,-soname,libsdma_gm_gm_peer_put_kernel.so \
  -L "${ASCEND_HOME_PATH}/lib64" \
  -Wl,-rpath,"${ASCEND_HOME_PATH}/lib64" \
  -o "${OUT_DIR}/libsdma_gm_gm_peer_put_kernel.so" \
...
"${BISHENG_BIN}" \
  ...
  -Wl,-rpath,"${OUT_DIR}" \
  -Wl,-rpath,"${ASCEND_HOME_PATH}/lib64" \
  -o "${OUT_DIR}/sdma_gm_gm_peer_put" \
  -lsdma_gm_gm_peer_put_kernel \
...
echo "== run PUT on devices ${DEVICE0} -> ${DEVICE1}"
cd "${OUT_DIR}"
...
LD_LIBRARY_PATH="${OUT_DIR}:${ASCEND_HOME_PATH}/lib64:${LD_LIBRARY_PATH:-}" \
  timeout --signal=KILL "${RUN_TIMEOUT:-90}" ./sdma_gm_gm_peer_put "${DEVICE0}" "${DEVICE1}"
rc=$?
set -e
if [ "${rc}" -eq 137 ]; then
