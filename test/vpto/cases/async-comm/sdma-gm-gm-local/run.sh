#!/bin/bash
# Build and run the local GM->GM SDMA case.
#
...
"${BISHENG_BIN}" \
  ...
  -shared -Wl,-soname,libsdma_gm_gm_local_kernel.so \
  -L "${ASCEND_HOME_PATH}/lib64" \
  -Wl,-rpath,"${ASCEND_HOME_PATH}/lib64" \
  -o "${OUT_DIR}/libsdma_gm_gm_local_kernel.so" \
...
"${BISHENG_BIN}" \
  ...
  -Wl,-rpath,"${OUT_DIR}" \
  -Wl,-rpath,"${ASCEND_HOME_PATH}/lib64" \
  -o "${OUT_DIR}/sdma_gm_gm_local" \
  -lsdma_gm_gm_local_kernel \
...
LD_LIBRARY_PATH="${OUT_DIR}:${ASCEND_HOME_PATH}/lib64:${LD_LIBRARY_PATH:-}" \
  timeout --signal=KILL "${RUN_TIMEOUT:-60}" ./sdma_gm_gm_local "${DEVICE_ID}"
rc=$?
set -e
if [ "${rc}" -eq 137 ]; then
