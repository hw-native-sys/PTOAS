...
#
# Build and run the cross-device peer-GM -> local-GM SDMA GET case.
#
...
#
# Needs two visible devices, and CANN 9.0.0 at run time: the workspace setup
# calls aclnnShmemSdmaStarsQuery, which earlier toolkits do not ship.
#
#   PTOAS_BUILD_DIR=... ARCH=a3 AICORE_ARCH=dav-c220-vec ./run.sh [dev0] [dev1]
#   PTOAS_BUILD_DIR=... ARCH=a5 AICORE_ARCH=dav-c310-vec  ./run.sh [dev0] [dev1]
set -euo pipefail
...
"${BISHENG_BIN}" \
  ...
  -shared -Wl,-soname,libsdma_gm_gm_peer_get_kernel.so \
  -L "${ASCEND_HOME_PATH}/lib64" \
  -Wl,-rpath,"${ASCEND_HOME_PATH}/lib64" \
  -o "${OUT_DIR}/libsdma_gm_gm_peer_get_kernel.so" \
...
"${BISHENG_BIN}" \
  ...
  -Wl,-rpath,"${OUT_DIR}" \
  -Wl,-rpath,"${ASCEND_HOME_PATH}/lib64" \
  -o "${OUT_DIR}/sdma_gm_gm_peer_get" \
  -lsdma_gm_gm_peer_get_kernel \
  -Wl,--allow-shlib-undefined -lruntime \
  -lstdc++ -lascendcl -lm -lc_sec -ldl -lnnopbase -lhccl -lpthread
...
echo "== run GET on devices ${DEVICE1} -> ${DEVICE0}"
cd "${OUT_DIR}"
# A kernel that never returns holds both devices until the process dies, and
# suspending the shell leaves them held. Kill the run instead, hard if it will
# not go, so a failed case costs a case rather than a card.
set +e
LD_LIBRARY_PATH="${OUT_DIR}:${ASCEND_HOME_PATH}/lib64:${LD_LIBRARY_PATH:-}" \
  timeout --signal=KILL "${RUN_TIMEOUT:-90}" ./sdma_gm_gm_peer_get "${DEVICE0}" "${DEVICE1}"
rc=$?
set -e
if [ "${rc}" -eq 137 ]; then
  echo "== killed after ${RUN_TIMEOUT:-90}s; devices ${DEVICE0}/${DEVICE1} may need a reset" >&2
fi
exit "${rc}"
