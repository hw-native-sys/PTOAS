...
#
# Where the transfer lands is arithmetic on the caller's own endpoints. The
# session cannot carry it: one session configures every post that uses it, so a
# per-transfer offset would be a per-transfer value in a shared configuration.
# The endpoints are i8, so an element offset is a byte offset.
ENGINE_SOURCE = f"""module attributes {{pto.target_arch = "{ARCH}", pto.kernel_kind = #pto.kernel_kind<vector>}} {{
  func.func @sdma_gm_gm_engine(
    ...
    %rec = pto.sdma_gm_gm %win_dst, %win_src, %nbytes session(%sess)
...
def engine_case(
    ...
    *,
    ...
    decides how many entries the queue should gain. ``offset`` shifts both
    endpoints, so the moved window keeps its position in the buffer.
    """
    if offset + nbytes > buffer_bytes:
        raise RuntimeError(
            f"case {name!r} would move past the end of the buffer: "
            f"{offset} + {nbytes} > {buffer_bytes}"
        )
    ...
    def make_case():
        ...
        # Everything outside the moved window stays zero. An expansion that
        # dropped the displaced endpoints would copy from the base and shift the
        # window, which shows up here rather than as a byte count that still
        # adds up.
        golden = np.zeros(buffer_bytes, dtype=np.uint8)
        window = slice(offset, offset + nbytes)
        ...
        return [src, session, out], expected, [offset, nbytes]
...
CASES = [
    ...
    engine_case(
        "sdma_gm_gm_engine_endpoint_offset",
        buffer_bytes=8192,
        nbytes=2048,
        block_bytes=2048,
        offset=2048,
