...
All four run the same kernel, and the first three differ only in the session
template the host writes, which is the point: how a transfer is carried is
described by session data, not by the kernel. `block_bytes` is what splits one
transfer into SQEs, so it decides how many entries the queue should gain. Where
the transfer lands is not session data -- one session configures every post that
uses it -- so the last case differs in the endpoints the kernel is handed
instead.
...
| `endpoint_offset` | 2048 bytes at offset 2048 of an 8 KiB buffer | 1 |
