...
def _max_scalar(lhs, rhs, *, fastmath=None):
...
def _min_scalar(lhs, rhs, *, fastmath=None):
...
# pto.max / pto.min / pto.abs intentionally reuse the builtin names in the public
# DSL namespace; bind the implementations through the module mapping so the
# definitions themselves do not shadow the Python builtins.
globals()['max'] = _max_scalar
globals()['min'] = _min_scalar
globals()['abs'] = _abs_scalar
...
def _emit_byte_offset(index_value, elem_type):
    ...
    index_type = IndexType.get()
    if not IndexType.isinstance(index_value.type):
        index_value = _pto_cast(index_value, index_type, index=True)
