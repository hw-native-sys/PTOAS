...
def _issue_1179_jit_decorator(name):
    """Shared ``pto.jit`` configuration for the issue-1179 probe kernels."""
    return pto.jit(
        name=name,
        kernel_kind="vector",
        target="a5",
...
def _make_issue_1179_simt_probe(name, source_dtype, target_dtype):
    ...
    @_issue_1179_jit_decorator(name)
    def probe(
        source: pto.ptr(source_dtype, "gm"),
        destination: pto.ptr(target_dtype, "gm"),
    ):
...
def test_scalar_cast_is_public():
    ...
    assert not hasattr(importlib.import_module("ptodsl"), "scalar")
