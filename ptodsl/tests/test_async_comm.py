...
class AsyncCommSurfaceTest(unittest.TestCase):
    def test_public_namespace_exports_the_surface(self):
        for name in ("dma_session_type", "session_init", "sdma_gm_gm"):
            with self.subTest(name=name):
                self.assertTrue(hasattr(pto, name), name)
        self.assertIn("dma_session_type", _types.__all__)

    def test_dma_session_type_names_its_engine(self):
        with make_context() as ctx, Location.unknown(ctx):
            self.assertEqual(
                str(pto.dma_session_type("sdma").resolve()),
                "!pto.dma_session<sdma>",
            )
            self.assertEqual(
                str(pto.dma_session_type("urma").resolve()),
                "!pto.dma_session<urma>",
            )
        with self.assertRaisesRegex(ValueError, "unknown engine"):
            pto.dma_session_type("nvlink")
    ...
    def test_surface_emits_session_init_and_sdma_gm_gm(self):
        ...
        self.assertIn("!pto.dma_session<sdma>", text)
    ...
    def test_session_init_hands_back_a_session_rather_than_filling_one(self):
        # The session is a result, not an out-parameter, and a post hands back
        # the record to wait on. Both are what make the type opaque: there is
        # nothing here for a kernel to declare or read back.
        with make_context() as ctx, Location.unknown(ctx):
            module = Module.create()
            with InsertionPoint(module.body):
                zero = pto.const(0, dtype=pto.i64)
                gm_ptr = pto.castptr(zero, pto.ptr(pto.i8, "gm"))
                sess = pto.session_init(gm_ptr)
                self.assertEqual(str(sess.type), "!pto.dma_session<sdma>")
                record = pto.sdma_gm_gm(gm_ptr, gm_ptr, 64, session=sess)
                self.assertEqual(str(record.type), "!pto.ptr<i64, gm>")
        ...
        with make_context() as ctx, Location.unknown(ctx):
            ...
            with InsertionPoint(module.body):
                ...
                with self.assertRaisesRegex(TypeError, "GM pointer"):
                    pto.session_init(ub_ptr)
                with self.assertRaisesRegex(ValueError, "unknown engine"):
                    pto.session_init(gm_ptr, engine="nvlink")
                # Anything that is not a session, including a pointer, is not a
                # session: the type is the whole check now.
                with self.assertRaisesRegex(TypeError, "pto.session_init"):
                    pto.sdma_gm_gm(gm_ptr, gm_ptr, 64, session=gm_ptr)
                with self.assertRaisesRegex(TypeError, "GM pointer"):
