...
unsetenv() {
  logandprint "[INFO]: Unset the environment path [ export ASCEND_PTO_AS_PATH=${relative_path_val}/${PTO_PLATFORM_DIR}]."
...
remote_all_soft_link() {
  ...
  local pto_as_lib_files="opapi_math opgraph_math ophost_math common_math"
  for lib_name in ${pto_as_lib_files}; do
