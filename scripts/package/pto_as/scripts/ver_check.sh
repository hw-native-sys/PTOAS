...
check_pto_version_file() {
  ...
  # pto_as/version.info -> pto_as
  elif [ -f "${_DEFAULT_INSTALL_PATH}/pto_as/version.info" ]; then
    ver_info="${_DEFAULT_INSTALL_PATH}/pto_as/version.info"
  else
    logandprint "[ERROR]: ERR_NO:${FILE_NOT_EXIST}; The [pto_as] version.info not exists."
  fi
  return
}
...
show_relation() {
  ...
  if [ "$relation_situation" = "SUCC" ]; then
    logandprint "[INFO]: Relationship of pto_as with ${req_pkg_name_val} in path ${req_pkg_path} check successfully"
    return 0
  else
    logandprint "[WARNING]: Relationship of pto_as with ${req_pkg_name_val} in path ${req_pkg_path} check failed. \
